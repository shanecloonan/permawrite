...
**CI gate (2026-08-15):** Tip CI `#31904265430` **in_progress** on B-303 `4e1fb020` — do not cancel. `#31902218314` rust **GREEN** / scripts **FAIL** (Seat A). Nightly `#31861932921` **GREEN**. Lane7 Path A **22565** / last_proven **22560**. **B-304** claim (transfer MFEX refuse). Slash-clone matrix frozen. Strategic path: L4 -> **B-40** -> **B-13a** -> **B-25**.
...
| **4** Protocol | **B-303** (`4e1fb020`) | **B-304** transfer MFEX refuse (claim base: `4e1fb020`) | After B-304: **B-35** still Phase 3 / B-25. After 2 hosts: live **B-32**. No B-297 clone | Lane 1 CI |
...
| B-304 | F5 P20: reject well-formed MFEX on non-storage (transfer) txs | 4+5 | **Claimed** (this commit); body after tip CI `#31904265430` GREEN |
...
1. **2026-08-15 - lane 4 - claim B-304 transfer MFEX refuse** (`4e1fb020`): `verify_transaction` / `build_transfer` still accept well-formed MFEX on a no-storage transfer, so a hostile wallet can partition vs honest empty extra. Claim base `4e1fb020`. Body after tip CI `#31904265430` GREEN. No B-13c; no DEFAULT flip; slash matrix frozen. Do not invent B-35. `[skip ci]`. *Observed (not staged):* `apply_block_proptest.rs`; `tx_storm.rs`; rc-audit json.
