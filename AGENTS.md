...
**CI #28845429505 GREEN** on `5221dad`. **Linux soak** run `28849489102` FAIL — hub tip reached 1 in `start-all` but WARMUP health-check required P2P sessions (fixed M2.5.65). **CI #28838850432 GREEN** on `934cc2f`. **Nightly #64 GREEN** (run `28841761235`).
...
| --- | --- | --- | --- |
| **1** | B-05 Linux soak PASS (M2.5.65 WARMUP fix) | **In progress** — re-dispatch soak | Release evidence on green stack (lane 2) |
