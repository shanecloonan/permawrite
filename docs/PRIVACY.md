...
**Path A lag honesty (B-218):** an F45 soft-pass means the Schnorr log verified at its max tip — **not** that the pin equals the live tip. When Path A republish lags (JOIN waves often see tip−log gaps of hundreds of heights), soft-pass is still a useful anti-eclipse check against the *published* attestation, but it is **not** exact-tip weak-subjectivity. Prefer a fresh Path A entry near tip (lane 7 republish) before treating the pin as day-of assurance; do not read `checkpoint_log_f45_soft_pass` as “synced to tip.”
