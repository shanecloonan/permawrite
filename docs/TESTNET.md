...
- `.github/workflows/ci.yml::public-devnet scripts` plus `scripts/ci-check.{sh,ps1}` — parse-check Bash and PowerShell public-devnet helpers before operator-script regressions can land.
...
- `mfn-node::p2p_light_follow_fetch` unit tests — outbound P2P light-follow rejects responses with more rows than requested or non-contiguous row heights before JSON/quorum use.
