# Lane 7: plan-only launch-status v8 schema rehearsal (Windows).
param(
    [switch]$PlanOnly
)
...
if ($launch.schema_version -ne "launch-status.v8") {
    throw "launch-status-rehearsal-smoke: expected launch-status.v8 got $($launch.schema_version)"
}
...
if ($launch.fraud_proof.validity_proof_phase -ne "4a") {
    throw "launch-status-rehearsal-smoke: fraud_proof.validity_proof_phase expected 4a"
}
...
Write-Host "  schema=launch-status.v8"
