...
[![Tests](https://img.shields.io/badge/tests-638_passing-brightgreen)](#status)
...
**Pre-network.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` binary exercises checkpoint persistence, optional JSON genesis (`--genesis`), solo **`step`** (mempool drain + coinbase + produce / apply / save, with **`--blocks N`** and optional **`--checkpoint-each`** for per-block checkpoints), an append-only **`chain.blocks`** log after each applied block (**M2.1.7**) with optional **validated replay vs checkpoint (M2.1.9)**, env-gated validator seeds, and **`serve`** (blocking TCP, one request line per connection: **`get_tip`** and **`submit_tx`** with **JSON-RPC 2.0** response lines — **M2.1.8** — and **`submit_tx`** **`params`** as either an object or a one-element hex array — **M2.1.8.1**); there is no P2P network, multi-method HTTP/WebSocket `rpc` server, or public testnet yet. As of the latest commit:
...
| **Total** | | **638** | Zero `unsafe`. Zero clippy warnings. Two ignored placeholders/doc examples. |
...
# Run the full workspace test suite (638 passing tests, ~6 min on a modern machine)
