...
**F45 tip race:** after pin+scan, `wallet light-scan --checkpoint-log` soft-passes when the tip raced past the log max (**B-161** in-CLI; prints `checkpoint_log_f45_soft_pass`). Wrappers remain for older binaries / ops: `light-scan-checkpoint-soft.sh` / `.ps1` (B-59/B-164; CI-gated by B-165). Browser/`mfn-wasm` does **not** soft-pass F45 (B-168) — refresh Path A or re-pin. Soft-pass does **not** skip Schnorr verify of the log or same-height disagreement; re-pin after a fresh Path A publish if you want exact-tip attestation.
...
**Tall-tip hang (B-250 / B-249):** Do **not** run bare `wallet light-scan` (or `light-scan-checkpoint-soft` without a pin) on a cold wallet at month-old tips (~16k) — it can hang while walking genesis→tip. Prefer the B-50 bootstrap helper first. `light-scan-checkpoint-soft.sh` / `.ps1` now detect an unpinned wallet and **delegate** to `bootstrap-wallet-from-checkpoint-log` (B-250) instead of hanging.

The read-only observer proxy at `http://5.161.201.73:8787/rpc` exposes public-safe methods only — use it for tip/header checks in a browser, never for wallet keys. Tall-tip `get_light_snapshot` / `get_block_headers` use a longer proxy timeout (**B-52** / F54; default 180s via `PROXY_HEAVY_RPC_TIMEOUT_MS`). At month-old tips (~16k), mfnd still re-reads the full `chain.blocks` log per cold header RPC (~3–4s); the proxy caches header rows and tip-warms them (**B-229**) so the public viewer does not abort mid-fetch. The wallet CLI mirrors that budget for `get_light_snapshot` (**B-161**; override with `MFN_HEAVY_RPC_TIMEOUT_MS`) so B-50 auto-bootstrap is not aborted by the ordinary 30s RPC I/O timeout. When the tip races past the log max after pin+scan, `light-scan --checkpoint-log` soft-passes F45 in-process (same honesty as B-59 / `light-scan-checkpoint-soft.sh`: Schnorr log verify still required). Prefer a local observer RPC for wallet bootstrap when possible.
