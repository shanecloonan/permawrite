...
- [`mfn-consensus/tests/block_apply.rs`](../mfn-consensus/tests/block_apply.rs) — M5 emit-order root, tampered-root-before-payout, provenance/treasury on accept, unknown commit / wrong chunk / duplicate rejects (`4e8ac41`); dual distinct proofs update both entries, body tamper rejects without state change (`9e5c129`).
