...
- **Validator finality evolution tests.** **M5 consensus (partial ✓):** `tests/validator_finality_evolution.rs` — pre-block `validator_root` quorum, liveness evolution atomicity, reject preserves state, bond/slashing-root mismatch rejects, validator-root movement on slash, unbond delay + settlement root semantics (`a97242a`, `267658d`).
