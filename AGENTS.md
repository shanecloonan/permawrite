...
| **3** Onboarding | **B-15 wave46 FUND FAIL** (liam F106; F45 lag=0) | **B-15** wave47 faucet-retry + formal JOIN archive assert (claim base: this head) | Human/assert SUMMARY; no Hetzner parallel JOIN | L4 checklist |
...
1. **2026-07-20 — lane 3 — B-15 wave46 FUND FAIL**: **liam** unfunded for upload (faucet 429; kate single-send→owned=1; iris F106 owned=1). Path A ckpt_max=4662 **lag=0** but hard scan still TIMEOUT. Honor §6. *Observed local work (not staged):* wallets, live-testnet-data*, other-lane dirty files.
