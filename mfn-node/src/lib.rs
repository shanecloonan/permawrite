...
//!   `127.0.0.1:18731`). **M2.3.3** / **M2.3.5** / **M2.3.6** / **M2.3.8** / **M2.3.9** / **M2.3.10** / **M2.3.11** / **M2.3.12** / **M2.3.13**: optional **`--p2p-listen ADDR`** binds a second TCP port, prints
...
//!   [`network::tcp_connect_peer_v1_handshake_with_tip_exchange`] and prints `mfnd_p2p_dial_ok=…` then **`mfnd_p2p_peer_tip`** then **`mfnd_p2p_height_cmp`** then **`mfnd_p2p_handshake_ms`** on success (each line shares the same **`hid=`**; **M2.3.13**). JSON-RPC stays on the main thread. Requests may omit `jsonrpc` (legacy); responses always
...
//!   Integration tests
//!   (`tests/mfnd_smoke.rs`, M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.3 + M2.3.4 + M2.3.5 + M2.3.6 + M2.3.7 + M2.3.8 + M2.3.9 + M2.3.10 + M2.3.11 + M2.3.12 + M2.3.13) drive `serve` over TCP
