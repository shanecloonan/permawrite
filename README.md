...
[![Tests](https://img.shields.io/badge/tests-582_passing-brightgreen)](#status)
...
**Pre-network.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` reference binary (M2.1.1) exercises checkpoint persistence from a real process; there is no P2P network, JSON-RPC surface, or public testnet yet. As of the latest commit:
...
| **Total** | | **582** | Zero `unsafe`. Zero clippy warnings. Two ignored placeholders/doc examples. |
...
# Run the full workspace test suite (582 passing tests, ~6 min on a modern machine)
