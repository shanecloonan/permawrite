...
| [`mfnd_serve`](src/mfnd_serve.rs) | **M2.1.6 + M2.1.8 + M2.1.8.1** blocking TCP control plane: `run_serve` accepts one UTF-8 line per connection, returns one **JSON-RPC 2.0** line, closes. Methods `get_tip` and `submit_tx` (hex `encode_transaction` bytes); **`submit_tx`** `params` may be `{"tx_hex":"…"}` or `["…"]`. |
| [`bin/mfnd`](src/bin/mfnd.rs) | **M2.1.1** process entrypoint calling [`mfn_node::mfnd_main`]; CLI in [`mfnd_cli`](src/mfnd_cli.rs). **M2.1.3** `step`; **M2.1.4** mempool drain + fee coinbase + `--blocks N`; **M2.1.5** `--checkpoint-each`; **M2.1.6** `serve` + `--rpc-listen`; **M2.1.7** `chain.blocks` on `step`; **M2.1.8** JSON-RPC responses on `serve`; **M2.1.8.1** array `params` for `submit_tx`. |
...
- **Unit (`mfnd_serve::tests`, M2.1.8 + M2.1.8.1)**: 13 tests — JSON-RPC envelope, `submit_tx` object + array `params`, empty array, bad `params[0]`, non-object/array `params`, missing `params`, and related guards.
...
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails, **`step --blocks 3`** reaches height 3 in one invocation, **`step --blocks 2 --checkpoint-each`** emits two `step_checkpoint` lines, **`serve --rpc-listen 127.0.0.1:0`** + TCP `get_tip` (`mfnd_serve_get_tip_over_tcp`) + **`mfnd_serve_get_tip_jsonrpc_echoes_id`**, **`serve`** `submit_tx` negative paths (`mfnd_serve_submit_tx_rejects_*`, **`mfnd_serve_submit_tx_array_params_rejects_bad_hex`**), **`mfnd_step_writes_block_log_then_serve_submit_tx_admits_transfer`** (synth-decoy genesis + `chain.blocks` + signed transfer admitted as `Fresh`).
