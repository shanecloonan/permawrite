...
**CI gate (2026-07-22):** **B-185** fix-forward #2 — mempool F7 dual-coinbase + heavy-RPC timeout env mutex after **CI `#29917551831`** ubuntu/windows FAIL. Full CI (cancels already-red tip).
...
| **3** Onboarding | **B-15 wave70** (lena last_proven=5842; faucet-F101b; F110; lag=544) | **B-15** wave71+ permanence density | Human SUMMARY; no Hetzner parallel JOIN | L4 checklist |
...
| **5** Privacy | **B-185** F7 floor + fixture/mempool/rpc fix-forwards (this commit); **B-182** (`e7b3e8bf`); **B-180** | *Idle* | After tip CI GREEN: pin run; After B-25: **B-35** / **B-37** / **B-19** | Doc-accuracy duty |
...
| B-185 | Low-level `build_transfer`/`build_storage_upload` fail-closed on `< WALLET_MIN_TX_INPUTS` (F7) | 5 | **Landed** (`617eb493`+`abe55dbc`+this tip) — mempool dual-fund + heavy-RPC env lock |
...
| B-183 | Ninth-offense asymmetric then absentee re-slash (early B-24bo) | 4 | **Claimed** — elevates B-171 after B-181 CI GREEN |
...
1. **2026-07-22 — lane 5 — B-185 fix-forward #2** (this commit): `mempool_integration` funds Alice with 2 coinbases (F7); `HEAVY_RPC_TIMEOUT_ENV_LOCK` serializes Windows env race in `mfn-cli` heavy-RPC timeout tests. Local mempool + rpc PASS. Cancels already-red **CI `#29917551831`**. Full CI (no skip). *Observed (not staged):* lane-4 `apply_block_proposals.rs`.
