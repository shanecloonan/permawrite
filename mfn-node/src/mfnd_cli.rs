//! Minimal `mfnd` command-line driver (M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13).
//!
...
//! **`serve`** (M2.1.6 + **M2.1.8** + **M2.1.10** + **M2.1.11** + **M2.1.12** + **M2.1.13**) binds a loopback TCP port and answers one
//! newline-delimited JSON request per connection (`get_tip`, `submit_tx`, `get_block`, `get_block_header`, `get_mempool`, `get_mempool_tx`)
    ...
    "usage: mfnd --data-dir <DIR> [OPTIONS] <COMMAND>\n\
     ...
     commands:\n\
       ...
       serve   load chain + empty mempool; TCP newline-delimited JSON-RPC 2.0 on --rpc-listen\n\
               (one request line per connection; methods: get_tip, submit_tx, get_block, get_block_header, get_mempool, get_mempool_tx;\n\
                ...
                get_mempool_tx params: {\"tx_id\":...} or [\"...\"] 64-char hex (32-byte tx id))\n"
}
