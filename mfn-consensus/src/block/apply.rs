...
pub fn apply_block(state: &ChainState, block: &Block) -> ApplyOutcome {
    ...
    for (ti, tx) in block.txs.iter().enumerate() {
        ...
        if tx_storage_ok && next.endowment_params.require_endowment_range_proof != 0 {
            let mut new_anchors: Vec<(usize, StorageCommitment, u64)> = Vec::new();
                ...
                match crate::extra_codec::parse_mfex_extra(&tx.extra) {
                    Ok(parsed) => {
                        if new_anchors.len() != parsed.endowment_range_proofs.len() {
                            ...
                            {
                                if !mfn_storage::verify_endowment_range_proof_wire(
                                    sc,
                                    *required,
                                    &wire.proof_bytes,
                                ) {
                                    errors.push(BlockError::EndowmentRangeProofInvalid {
                                        tx: ti,
                                        output: *oi,
                                    });
                                }
                    ...
                    Err(e) => {
                        errors.push(BlockError::EndowmentRangeProofParse {
                            tx: ti,
                            reason: e.to_string(),
                        });
                    }
