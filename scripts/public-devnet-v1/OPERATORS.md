...
| **B-42 / B-248 invite-load** | `bash scripts/public-devnet-v1/invite-load-smoke-rehearsal.sh --apply` (or `.ps1 -Apply`) — B-15-safe preflight (proxy+faucet+seeds); `serialize-with-reason` until clear. Live JOIN arm: `MFN_INVITE_LOAD_ALLOW_LIVE=1` + `--live` (JOIN still operator-driven; never during B-15) |
