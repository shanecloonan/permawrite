...
- **apply_block proptest (M5.2–M5.6+)** (`tests/apply_block_proptest.rs`, 26 tests) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain).
