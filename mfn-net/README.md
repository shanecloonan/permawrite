...
- **`block_sync`** — **`GetBlocksByHeightV1`** / **`BlocksV1`** request/response (**M2.3.18**) and **`pull_blocks_to_tip`** catch-up when the remote tip is ahead (**M2.3.19**). Catch-up now enforces exact next-height advancement for every applied response block, and **`recv_blocks_v1`** skips interleaved proposal/vote/gossip frames while waiting for a **`BlocksV1`** reply (live **`--produce`** sessions).
