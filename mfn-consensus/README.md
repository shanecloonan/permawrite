...
**Tests:** 373 passing (151 unit + 222 integration across six `tests/` targets; 25 nightly `#[ignore]`) &nbsp;·&nbsp; **`unsafe`:** forbidden &nbsp;·&nbsp; **Clippy:** `-D warnings` (see `scripts/ci-check`)
...
- **Emission simulation (M5.0–M5.34)** (`tests/emission_simulation.rs`, 19 default + 18 nightly `#[ignore]`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains (**64-block** validator mixed CLSAG+SPoRA in CI, **M5.34/B-03**), liveness-slash, combined-inflow, 32/64-block equivocation combined-inflow ledgers, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
