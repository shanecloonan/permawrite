...
- **`tests/integration.rs` (Agent 1 legacy mixed integration, `ba04799` / `6422d1a`)** — `legacy_mixed_clsag_fee_and_storage_proof_at_genesis_plus_block1` proves legacy CLSAG fee + SPoRA forward-apply with treasury identity in a real block flow; `legacy_mixed_two_block_treasury_ledger_identity` carries the same privacy-fee + SPoRA treasury credit/drain identity across two blocks.
