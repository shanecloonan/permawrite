...
/// Build a SPoRA proof from local file bytes and submit via `submit_storage_proof`.
pub fn operator_prove(
    ...
    let data = std::fs::read(data_path)
        .map_err(|e| OperatorCmdError::Usage(format!("read {}: {e}", data_path.display())))?;
    ...
    if tree.root() != commit.data_root {
        return Err(OperatorCmdError::Usage(
            "file bytes do not match on-chain data_root (wrong payload or chunk_size)".into(),
        ));
    }
