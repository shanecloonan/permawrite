...
/// B-163 (early B-24bd): B-154 arc through seventh dual slash + dual settle reset, then
/// advance past the proof-reward window and climb to an **eighth** dual empty-audit slash, then dual-settle drain.
/// Elevates B-155 (seventh-slash→dual settle) to eighth-slash→dual settle. Does **not** close full **B-24**.
#[test]
fn b163_b5_eighth_dual_slash_then_dual_settle_drain_identity() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
    ...
    // Dual settle drains eighth-offense slash credit.
    let scratch = build_unsealed_header(&st, &[], &[], &[], &[], slot, 1_000);
    let proofs =
        b5_two_op_proofs_for_mask(&gen.built, &gen.payload, &scratch.prev_hash, slot, 0b11);
    let settlements =
        storage_proof_operator_settlements(&proofs, &st.storage, slot, &st.endowment_params);
    assert_eq!(settlements.len(), 2, "both settle after eighth slash");
    let bonus_total: u128 = settlements
        .iter()
        .map(|(_, b)| *b)
        .fold(0, u128::saturating_add);
    ...
    match apply_block(&st, &blk) {
        ApplyOutcome::Ok { state, .. } => {
            assert_eq!(
                state.treasury, expected_treasury,
                "eighth-offense slash-funded treasury then dual SPoRA drain"
            );
            assert_eq!(
                state.storage_operator_stats[&gen.id0].consecutive_missed_audits,
                0
...
/// B-166 (early B-24be): B-154 arc through seventh dual slash + dual settle reset, then
/// advance past the proof-reward window and climb to an **eighth** dual empty-audit slash, then asymmetric (op0-only) settle drain.
/// Elevates B-156 (seventh→asymmetric settle) to eighth→asymmetric settle. Does **not** close full **B-24**.
#[test]
fn b166_b5_eighth_dual_slash_then_asymmetric_settle_drain_identity() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
    ...
    // Asymmetric settle: only op0 drains eighth-offense slash credit.
    let scratch = build_unsealed_header(&st, &[], &[], &[], &[], slot, 1_000);
    let proofs =
        b5_two_op_proofs_for_mask(&gen.built, &gen.payload, &scratch.prev_hash, slot, 0b01);
    let settlements =
        storage_proof_operator_settlements(&proofs, &st.storage, slot, &st.endowment_params);
    assert_eq!(settlements.len(), 1, "only op0 settles after eighth slash");
    let bonus = settlements[0].1;
    let storage_drain = u128::from(emission.storage_proof_reward).saturating_add(bonus);
    let expected_treasury = st.treasury.saturating_sub(storage_drain.min(st.treasury));
    let unsealed = build_unsealed_header(&st, &[], &[], &[], &proofs, slot, 1_000);
    ...
    match apply_block(&st, &blk) {
        ApplyOutcome::Ok { state, .. } => {
            assert_eq!(
                state.treasury, expected_treasury,
                "eighth-offense slash-funded treasury then asymmetric SPoRA drain"
            );
            assert_eq!(
                state.storage_operator_stats[&gen.id0].consecutive_missed_audits, 0,
                "prover miss resets"
...
/// B-169 (early B-24bf): B-166 twin — eighth dual slash then op1-only settle drains
/// eighth-offense slash credit. Completes eighth-offense asymmetric settle pair. Elevates B-157.
#[test]
fn b169_b5_eighth_dual_slash_then_op1_asymmetric_settle_drain_identity() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
    ...
    // Asymmetric settle: only op1 drains eighth-offense slash credit.
    let scratch = build_unsealed_header(&st, &[], &[], &[], &[], slot, 1_000);
    let proofs =
        b5_two_op_proofs_for_mask(&gen.built, &gen.payload, &scratch.prev_hash, slot, 0b10);
    let settlements =
        storage_proof_operator_settlements(&proofs, &st.storage, slot, &st.endowment_params);
    assert_eq!(settlements.len(), 1, "only op1 settles after eighth slash");
    let bonus = settlements[0].1;
    let storage_drain = u128::from(emission.storage_proof_reward).saturating_add(bonus);
    let expected_treasury = st.treasury.saturating_sub(storage_drain.min(st.treasury));
    let unsealed = build_unsealed_header(&st, &[], &[], &[], &proofs, slot, 1_000);
...
/// B-170 (early B-24bg): B-162 arc through eighth dual slash, then empty both-miss — no
/// treasury drain; both miss streaks restart at 1. Elevates B-158. Closes eighth-offense
/// prove matrix {00,01,10,11} with B-163/B-166/B-169.
#[test]
fn b170_b5_eighth_dual_slash_then_empty_both_miss_no_drain_identity() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
    ...
    // Empty after eighth slash: no drain; both miss restart at 1.
    let treasury_after_eighth = st.treasury;
    let last_proven_before = st.storage.get(&ch).expect("entry").last_proven_slot;
    st = apply_empty_at_audit_slot(&st, slot);
    assert_eq!(
        st.treasury, treasury_after_eighth,
        "empty after eighth slash must not drain treasury"
    );
    assert_eq!(
        st.storage_operator_stats[&gen.id0].consecutive_missed_audits, 1,
        "op0 miss restarts at 1"
...
/// B-171 (early B-24bh): B-154 arc through seventh dual slash + dual settle reset, then
/// advance past the proof-reward window and climb to an **eighth** dual empty-audit slash, then asymmetric settle, window-spaced absentee climb, and re-slash while peer settles.
/// Elevates B-159 (seventh asymmetric→absentee re-slash) to eighth-offense funding. Does **not** close full **B-24**.
#[test]
fn b171_b5_eighth_offense_asymmetric_then_absentee_reslash_while_peer_settles() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
...
/// B-173 (early B-24bi): B-154 arc through seventh dual slash + dual settle reset, then
/// advance past the proof-reward window and climb to an **eighth** dual empty-audit slash, then op1 asymmetric settle, window-spaced absentee climb, and re-slash while peer settles.
/// B-171 twin; elevates B-160 (seventh asymmetric→absentee re-slash) to eighth-offense funding. Does **not** close full **B-24**.
#[test]
fn b173_b5_eighth_offense_op1_asymmetric_then_absentee_reslash_while_peer_settles() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
...
/// B-175 (early B-24bj): B-162 arc through eighth dual slash + dual settle reset, then
/// advance past the proof-reward window and climb to a **ninth** dual empty-audit slash.
/// Elevates B-162 settle-reset re-arm (seventh→eighth) to eighth→ninth. Does **not** close full **B-24**.
#[test]
fn b175_b5_settle_reset_then_ninth_dual_slash_treasury_identity() {
    let gen = genesis_with_b5_two_operators();
    let mut st = gen.state;
    let cap = st.endowment_params.operator_audit_missed_cap;
    let slash_bps = st.endowment_params.operator_slash_bps;
    ...
    // Offense 9: re-climb from settle-reset miss=0, then dual slash again.
    for i in 0..(cap - 1) {
        st = apply_empty_at_audit_slot(&st, slot);
        assert_eq!(
            st.treasury, model,
            "pre-ninth-slash climb must not change treasury {i}"
        );
        assert_eq!(
            st.storage_operator_stats[&gen.id0].consecutive_missed_audits,
            i + 1,
            "op0 miss after eighth settle-reset {i}"
        );
        assert_eq!(
            st.storage_operator_stats[&gen.id1].consecutive_missed_audits,
            i + 1,
            "op1 miss after eighth settle-reset {i}"
        );
        slot = slot.saturating_add(1);
    }
    (model, bond0) = treasury_after_b5_slash(model, bond0, slash_bps);
    ...
    assert_eq!(
        st.treasury, model,
        "ninth dual slash credits both forfeitures on post-settle bonds"
    );
    assert_eq!(
        st.storage_operator_stats[&gen.id0].consecutive_missed_audits,
        0
    ...
    assert!(
        st.storage_operators.contains_key(&gen.id0) && st.storage_operators.contains_key(&gen.id1),
        "partial ninth slash must keep both registered"
    );
    assert!(st.treasury > 0, "ninth-offense credit spendable");
}
