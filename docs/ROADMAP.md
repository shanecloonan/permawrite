...
**As of 2026-07-19** (planning head pending; code head `5dc3aa8` B-29; **B-16** `49d28f9`; **B-30** `2f1b4e2`; experimental public testnet live on Hetzner `5.161.201.73`).
...
| **L1** | Software-ready RC | Green CI + Nightly + `release-evidence` **go** on exact head | ✓ baseline (`b4a3fa7`); B-34 lineage `#29711500087` on `76d4f04` → current `#29711605173` on `e10a8b3` (B-29 inclusive); **Nightly RED** `#29701967243` (**B-29**) |
...
| **L4** | Hardened public testnet | TL-9 go/no-go + named watchers + B-15 outside-in evidence + privacy docs match shipped UX | **In progress** (B-16 ✓; B-30 ✓; B-15 / B-29 / B-26 / B-27 / B-31 open) |
...
### Public devnet genesis baseline (canonical spec)

Source: [`mfn-node/testdata/public_devnet_v1.json`](../mfn-node/testdata/public_devnet_v1.json) (not under `scripts/public-devnet-v1/`). Editing after TL-8 starts a **new chain** (`genesis_id` mismatch).

| Field | Value (2026-07-19) | Roadmap note |
|---|---|---|
| `operator_salted_challenges` | `1` | ✓ **B3 genesis on** — operator-salted SPoRA active |
| `require_registered_operators` | `1` | ✓ registry enforced at `apply_block` |
| `require_endowment_range_proof` | `1` | ✓ **B-11** on devnet |
| `min_storage_operator_bond` | `0` | Bonds optional — **PM1** scale-up in Phase 4 (Path B) |
| `operator_slash_bps` | `250` | ✓ **B5** slashing wired; bonds `0` limits slash economics |
| `subsidy_to_treasury_bps` | absent → defaults `0` | **B-13** fork pending ([`FEES.md` §5.4](./FEES.md)) |

**Still open for permanence depth:** tail subsidy (**B-13**), multi-operator internet proof evidence (**B3 multi-op**), PM3 lottery, PM2 replication accounting, 30d treasury soak.
...
| **B-15** | Outside-in JOIN_TESTNET rehearsal on VPS + archived evidence | 3 | **Tooling shipped** (`774320f`, R-1–R-4, `73abf77`/`02c8df8` tall-tip checkpoint log); **live VPS transcript pending** — do not restart `faucet-http` or run parallel rehearsals during evidence capture (AGENTS §6) |
...
|---|---|---|---|
| **L1 CI** | GitHub CI on B-29-inclusive head (`#29711500087` → `#29711605173` on `e10a8b3`) | 1 | Matrix **GREEN** (use **B-34** if queued stalls) |
...
| **B-41** | Public P2P bind repair on Hetzner | 7+2 | Outside dial to seeds PASS; after B-15 lock |
| **B-42** | Invite-load smoke (see work package) | 3+7 | Evidence archived; after B-15/B-41; before TL-9 |
...
|---|---|---|
| [`JOIN_TESTNET.md`](./JOIN_TESTNET.md) | ✓ | HTTP faucet dual-send + job poll; `light-scan --checkpoint-log` at high tip; observer proxy read-only |
| [`TESTNET.md`](./TESTNET.md) | ✓ | Live public wallet path + local-mesh fund-wallet F7 |
| [`PRIVACY.md`](./PRIVACY.md) | ✓ | F7 floors + why light-scan is default at high tip; F12 checkpoint pointer |
| [`PRIVACY_HARDENING.md`](./PRIVACY_HARDENING.md) | ✓ | B-16 marked shipped in prioritization table |
| [`CHECKPOINT_LOG.md`](./CHECKPOINT_LOG.md) | ✓ | Canonical F12 reference |
| [`TESTNET_INVITE.md`](./TESTNET_INVITE.md) | ✓ | Faucet URL/cooldown + privacy defaults + proxy disclaimer |
| [`scripts/public-devnet-v1/OPERATORS.md`](../scripts/public-devnet-v1/OPERATORS.md) | ✓ | R-1–R-4 faucet behavior; no parallel rehearsals during B-15 |
...
#### B-81 — full-slash deregister while peer settles (lane 4; early B-24e)
...
#### B-86 — slash-funded treasury then dual-settle drain (lane 4; early B-24g)
...
#### B-99 — slash-funded treasury then empty both-miss (lane 4; early B-24j)
...
#### B-102 — slash-funded op1 asymmetric then absentee re-slash while peer settles (lane 4; early B-24l)
...
#### B-104 — second dual-slash then dual-settle drain (lane 4; early B-24n)

Run two successive dual empty-audit slashes (B-103), then both operators settle (`mask=0b11`) on the next audit slot. Treasury identity: second-offense slash credits fund dual SPoRA drain (rewards + bonuses); bonds stay at post-second-slash amounts; miss streaks clear; `last_proven_slot` advances. Complements B-103 (repeat slash only) and B-86 (first-slash→settle). Does **not** close full **B-24**.

#### B-105 — second dual-slash then asymmetric settle drain (lane 4; early B-24o)

Run two successive dual empty-audit slashes (B-103), then only op0 settles (`mask=0b01`). Treasury drains for one settlement; prover miss=0; absentee miss=1; bonds stay post-second-slash. Twin settle corner of B-104. Complements B-95 (first-slash asymmetric) and B-104 (second-slash dual settle). Does **not** close full **B-24**.

#### B-106 — second dual-slash then op1 asymmetric settle drain (lane 4; early B-24p)

Symmetric twin of B-105: two successive dual empty-audit slashes, then only op1 settles (`mask=0b10`). Treasury drains for one settlement; prover (op1) miss=0; absentee (op0) miss=1; bonds stay post-second-slash. Completes the second-offense asymmetric settle pair. Does **not** close full **B-24**.

#### B-107 — second dual-slash then empty both-miss (lane 4; early B-24q)
...
#### B-109 — third dual-slash then dual-settle drain (lane 4; early B-24s)
...
#### B-111 — third dual-slash then op1 asymmetric settle drain (lane 4; early B-24u)
...
#### B-113 — third-offense asymmetric then absentee re-slash while peer settles (lane 4; early B-24w)

B-108 arc through a third dual empty-audit slash, then the B-101 path: op0-only settle, window-spaced climb of absentee miss to `cap-1`, then op0 settles again while op1 alone re-slashes. Treasury identity: absentee re-slash credit then peer SPoRA drain; prover miss stays 0; re-slash resets absentee; bonds track modeled partial slash; `last_proven_slot` advances. Complements B-101 (first-slash funding) and B-110 (stops at asymmetric). Does **not** close full **B-24**.

#### B-114 — third-offense op1 asymmetric then absentee re-slash while peer settles (lane 4; early B-24x)

Symmetric twin of B-113: B-108 arc through a third dual slash, then op1-only settle, window-spaced climb of absentee (op0) miss to `cap-1`, then op1 settles again while op0 alone re-slashes. Completes the third-offense asymmetric re-slash pair (B-113/B-114). Does **not** close full **B-24**.

#### B-115 — second-offense asymmetric then absentee re-slash while peer settles (lane 4; early B-24y)

Two successive dual empty-audit slashes (B-103), then the B-101 path: op0-only settle, window-spaced climb of absentee miss to `cap-1`, then op0 settles again while op1 alone re-slashes. Fills the second-offense gap between B-101 (first-offense funding) and B-113 (third-offense funding). Complements B-104/B-105 (second-slash settle matrix). Does **not** close full **B-24**.

#### B-116 — second-offense op1 asymmetric then absentee re-slash while peer settles (lane 4; early B-24z)
...
#### B-118 — fourth dual-slash then dual-settle drain (lane 4; early B-24ab)
...
#### B-120 — fourth dual-slash then op1 asymmetric settle drain (lane 4; early B-24ad)
...
#### B-122 — fourth-offense asymmetric then absentee re-slash while peer settles (lane 4; early B-24af)

B-117 arc through fourth dual slash, then the B-101 path: op0-only settle, window-spaced climb of absentee miss to `cap-1`, then op0 settles again while op1 alone re-slashes. Elevates B-113/B-115 to fourth-offense funding. Does **not** close full **B-24**.

#### B-124 — fourth-offense op1 asymmetric then absentee re-slash while peer settles (lane 4; early B-24ag)

B-122 twin: B-117 arc through fourth dual slash, then B-102 path — op1-only settle, window-spaced climb of absentee (op0) miss to `cap-1`, then op1 settles again while op0 alone re-slashes. Completes fourth-offense asymmetric re-slash pair. **B-123** is lane1 soak id — skipped. Does **not** close full **B-24**.

#### B-126 — settle-reset then fifth dual-slash treasury identity (lane 4; early B-24ah)

B-117 elevated: fourth dual slash + dual settle reset, then advance past the proof-reward window and climb to a fifth dual empty-audit slash. Proves settle-reset re-arm across a third post-settle offense cycle. Does **not** close full **B-24**.

#### B-128 — fifth dual-slash then dual-settle drain (lane 4; early B-24ai)

B-126 arc through fifth dual slash, then both operators settle (`mask=0b11`) — fifth-offense slash credits fund dual SPoRA drain. Complements B-126 (stops at fifth slash) and B-118 (fourth-slash→dual settle). Does **not** close full **B-24**.

#### B-130 — fifth dual-slash then asymmetric settle drain (lane 4; early B-24aj)

B-126 arc through fifth dual slash, then only op0 settles (`mask=0b01`) — fifth-offense slash credits fund a single SPoRA drain; absentee restarts miss=1. Complements B-128 (dual settle) and B-119 (fourth-slash asymmetric). **B-129** is lane1 tip-ckpt lag id. Does **not** close full **B-24**.

#### B-131 — fifth dual-slash then op1 asymmetric settle drain (lane 4; early B-24ak)

B-130 twin: B-126 arc through fifth dual slash, then only op1 settles (`mask=0b10`) — fifth-offense slash credits fund a single SPoRA drain; absentee (op0) restarts miss=1. Completes the fifth-offense asymmetric settle pair. Does **not** close full **B-24**.

#### B-132 — fifth dual-slash then empty both-miss (lane 4; early B-24al)
...
#### B-143 — fifth-offense op1 asymmetric then absentee re-slash while peer settles (lane 4; early B-24an)

B-142 twin: B-126 arc through fifth dual slash, then B-102 path — op1-only settle. Completes fifth-offense asymmetric re-slash pair. Does **not** close full **B-24**.

#### B-147 — settle-reset then sixth dual-slash treasury identity (lane 4; early B-24ao)

B-126 arc through fifth dual slash + dual settle reset, then advance past the proof-reward window and climb to a **sixth** dual empty-audit slash. Elevates B-126/B-117 settle-reset re-arm. Does **not** close full **B-24**.

#### B-148 — sixth dual-slash then dual-settle drain (lane 4; early B-24ap)
...
#### B-150 — sixth dual-slash then op1 asymmetric settle drain (lane 4; early B-24ar)
...
#### B-152 — sixth-offense asymmetric then absentee re-slash (lane 4; early B-24at)
...
#### B-155 — seventh dual-slash then dual-settle drain (lane 4; early B-24aw)

B-154 arc through seventh dual slash, then both operators settle (`mask=0b11`). Elevates B-148.

#### B-156 — seventh dual-slash then asymmetric settle drain (lane 4; early B-24ax)

Elevates B-149. Does **not** close full **B-24**.

#### B-157 — seventh dual-slash then op1 asymmetric settle drain (lane 4; early B-24ay)

B-156 twin: B-154 arc through seventh dual slash, then only op1 settles (`mask=0b10`). Completes seventh-offense asymmetric settle pair. Elevates B-150. Does **not** close full **B-24**.

#### B-158 — seventh dual-slash then empty both-miss (lane 4; early B-24az)
...
**Gate:** 30+ day VPS soak with stable height, matched uploads/proofs, treasury telemetry within modeled bounds, no silent permanence regressions in [`PERMANENCE_HARDENING.md`](./PERMANENCE_HARDENING.md). Recorded as **B-25**.

#### Phase 1 permanence playbook (acceptance criteria)

Ordered after L4. Permanence first — do not start Tier 2 (Phase 3) or Path B value (Phase 4) until **B-25** or an explicit human waiver.
...
| **Now** | **B-34** watch CI `#29711605173` on `e10a8b3` (lineage `#29711500087` on `76d4f04`; escalate if runner-starved twice) | 1 / planning | — |
...
| **Week close** | Written week-1 note: sims status, multi-op evidence status, treasury baseline vs live tip | 6+7 | Note archived under `scripts/public-devnet-v1/evidence/` or OPERATORS |

**Hard rules:** do not enable `subsidy_to_treasury_bps` on Path A until **B-33**; do not claim **B-25** from a single week of data; do not restart `faucet-http` if B-15 capture is somehow still open.

**Pass:** B-13a claimed and progressing (or GREEN); B-32 armed; telemetry baseline archived; no Tier-2/Path-B drift in §5/§8.

#### B-42 — Invite-load smoke (lanes 3+7 — before TL-9)

Prove the outsider invite path survives concurrent light load **before** circulating [`TESTNET_INVITE.md`](./TESTNET_INVITE.md). Complements **B-15** (single JOIN transcript) and **B-27** (fresh soak/participant) — does **not** replace either.

| Check | Pass when |
|---|---|
| **Concurrent JOIN** | ≥2 parallel `join-testnet-rehearsal` / JOIN paths against live faucet+observer (stagger if faucet lock requires) — both reach `SUMMARY: PASS` or documented serialize-with-reason |
| **Faucet under load** | R-4 peer-IP rate limit behaves; no unbounded in-memory job growth; cooldown honored |
| **Observer proxy** | Read-only proxy stays healthy; light-scan + checkpoint-log path succeeds under parallel clients |
| **Front / invite URLs** | Documented invite URLs respond; no auth/RPC leakage beyond threat model |
| **Conflict** | Respect §6 faucet lock during **B-15** capture — run B-42 **after** B-15/B-41 window or on non-conflicting schedule |
| **Evidence** | `invite-load-smoke-<date>.txt` (or agreed pattern) archived; cite SHA + CI head in §8 |

**Pass:** Evidence archived; no P0 invite-path failures; residual limits noted in OPERATORS before TL-9.

#### B-43 — Path B genesis freeze inventory (lane 7 — before L5)

Freeze **what must be locked** before any Path B / incentivized genesis ceremony. Inventory only — does **not** run the ceremony. See [`TESTNET_GENESIS_CEREMONY.md`](./TESTNET_GENESIS_CEREMONY.md) Path B.

| Freeze item | Owner | Locked when |
|---|---|---|
| **Constitution / ring / endowment floors** | 4+6 | Written list of invariants that Path B must preserve (no silent weakening) |
| **Header v2 / `utxo_root` in BLS bytes** | 4 | Decision: enable `header_version: 2` on new chain only (yes/no + rationale) |
| **Validator / BLS PoP material** | 7+human | Ceremony roles named; offline seed handling procedure; never paste keys in chat |
| **`require_validator_bls_pop` + genesis JSON shape** | 4+6 | Spec fields + verify command pinned (`genesis-validator-bls-pop.sh --verify`) |
| **Economics levers** | 6 | Which of B-13c / PM1 bonds / fee params land on Path B vs stay Path A |
| **Topology** | 7 | Role-separated VPS inventory (validator / operator / observer hosts) per [`REFERENCE_TOPOLOGY.md`](./REFERENCE_TOPOLOGY.md) |
| **Wipe + re-soak plan** | 7 | Data-dir wipe, TL-5/TL-6 re-run checklist, invite/docs republication |
| **Sign-off cells** | human | Named approvers for Path B decision template |

**Hard rules:** Path A toy keys stay non-value; do not start Path B until **B-25** (or explicit human waiver); inventory is a gate doc, not a fake L5 claim.

**Pass:** Freeze inventory committed (ROADMAP pointer + OPERATORS / ceremony checklist); human cells named or explicitly TBD with owner.

#### B-44 — PM3 work package (lane 4+6 — after B-32)

Windowed SPoRA lottery vs first-to-publish latency race ([`PROBLEMS.md` §6](./PROBLEMS.md#6-spora-proof-winning-is-a-pure-first-to-publish-latency-race), [`F5.md`](./F5.md) PM3, [`DECENTRALIZATION.md`](./DECENTRALIZATION.md)). **Permanence first** — colocation-favored races are a permanence risk. Arm only after **B-32** multi-op evidence proves ≥2 real operators exist.
...
| **Docs** | PROBLEMS §6 / PERMANENCE_HARDENING / OPERATORS payout notes updated to match shipped behavior |
| **Does not include** | PM2 replication accounting (follows); PM1 bond scale-up (Phase 4); Tier 2 privacy |

**Pass:** Consensus tests GREEN; multi-op lottery behavior documented; unblocks honest sequencing into **B-24** settlement audit + **PM2**; note run IDs in §8.
...
| **B-20** Producer↔treasury runway coupling | Phase 1 | 6 |
| **B-21** Dandelion++ internet soak evidence | Phase 3 | 1 |
| **B-22** TL-8 checkpoint log VPS publish verify | Phase 0 | 7 |
...
| **B-40** First permanence week (arm day-of L4) | Phase 1 | 6 |
| **B-41** Repair public P2P binds on Hetzner | Phase 0 | 7+2 |
| **B-42** Invite-load smoke before TL-9 | Phase 0 | 3+7 |
| **B-43** Path B genesis freeze inventory | Phase 4 | 7+human |
| **B-44** PM3 windowed SPoRA lottery work package | Phase 1 | 4+6 |
...
[B-15 VPS evidence + assert] ──→ [B-41 P2P bind repair] ──→ [B-26 R-4 faucet deploy]
        ↓ (B-16 ✓ shipped)
[B-27 fresh soak] [B-22 checkpoint] [B-30 risk owners] [B-31 threat posture] [B-42 invite-load smoke]
        ↓
[TL-9 go/no-go + invites]  (= L4)
        ↓
[B-40 first permanence week] ∥ [B-13a] ──→ [B-33] ──→ [B-13c] ──→ [B-28 Treasury watch]
        ↓ parallel (day-of L4; do not block B-13a)
[B-32 multi-op assert] → [B-44 PM3] → [B-24] → [PM2] → [B-38] → [B-25 permanence go/no-go]
...
[Tier 2 + B-35 + B-37 hidden fees + B-19]
        ↓
[B-43 Path B genesis freeze] ──→ [TL Path B + Header v2 + multi-VPS] ──→ [B-17 + adversarial]  (= L5)
        ↓
...
| **P16** | Cover traffic / uniform broadcast timing | Privacy | **B-21** Dandelion++ soak evidence |
| **P21** | SPoRA proof submission timing obfuscation | Privacy | PM3 lottery design |
...
| **M2.4.6** | Strict devnet `health-check`: shared `tip_height`/`tip_id` + `genesis_id` across hub and voters. | ✓ shipped |
...
| **M2.4.60** | Public-devnet local-mesh liveness: soak converged warmup (`soak: WARMUP`), manifest multi-producer sortition bounds; helper mesh role docs for all-produce vs hub+committee. | ✓ shipped |
...
| **M2.4.77** | Auto-dispatch Nightly after green CI (`rc-validation-after-ci.yml`); bash `--archive-evidence`; nightly jobs archive evidence. | ✓ shipped |
...
| **M2.4.80** | Fix `validate-workflow-encoding.sh` grep null-byte false positive (python3 byte check). | ✓ shipped |
...
| **M3.40** | Support-bundle node status: capture `get_status` JSON for RPC, genesis, tip, and P2P triage alongside wallet/permanence records. | ✓ shipped |
...
| **M7.9** | Ignored smoke: three-validator produce mesh, hub inbox + **M7.5** auto fan-out to both committee voters (no `push-chunks`; `chunk_p2p_three_validator_produce_smoke` auto_fanout test). | ✓ shipped |
...
- **Proptest fuzzing of `apply_block`.** Randomized inputs; reject any panic / inconsistency. Target: 24-hour fuzz campaign with no findings. **M5.2 (partial ✓):** `tests/apply_block_proptest.rs` — 32-case CI props for valid empty chains + header tamper rejects with state unchanged; **128-block empty chain in CI (M5.37)**; all deep proptest chains in default CI (**M5.36–M5.39**). **M5.2+ (partial ✓):** storage-proof chains, signed `BondOp::Register` chains, forged-register + duplicate-proof rejects; **32-block SPoRA proof chain in CI (M5.37)**. **M5.2++ (partial ✓):** alternating empty + SPoRA proof chains (`prop_alternating_empty_and_storage_chains`). **M5.4 (partial ✓):** alternating bond-register (treasury credit) + SPoRA proof (treasury drain) proptests with ledger identity; multi-chunk storage fixture; **epoch churn-cap deep chain in CI (M5.39)** (`deep_alternating_register_storage_treasury_8`). **M5.5 (partial ✓):** `prop_mixed_clsag_fee_and_storage_proof_treasury` — CLSAG fee + SPoRA proof in the **same block** with treasury ledger identity (deterministic ring); **64-block in CI (M5.36)**. **M5.6 (partial ✓):** validator quorum + coinbase + CLSAG fee + SPoRA proof same-block proptest (`prop_validator_mixed_clsag_fee_and_storage_proof_treasury`); **32-block in CI (M5.37)**. **M5.7 (partial ✓):** bond inflow + randomized CLSAG fees + SPoRA outflow proptest (`prop_bond_inflow_random_fee_and_proof_outflow_treasury`, `dde886e`). **M5.8 (partial ✓):** validator combined inflow randomized fee treasury proptest (`prop_validator_combined_inflow_random_fee_treasury`, `a858e54`) and equivocation combined-inflow random-fee treasury proptest (`prop_validator_equivocation_combined_inflow_random_fee_treasury`, `1ae43ca`). **M5.16/M5.17 (partial ✓):** validator combined inflow random-schedule treasury proptest with equivocation and no-equivocation variants (`prop_validator_combined_inflow_random_schedule_treasury`, `prop_validator_combined_inflow_random_schedule_no_equivocation_treasury`; `e9d1c44`, `cf06280`). **M5 rollback (partial ✓):** `reject_duplicate_storage_proof_in_mixed_block_without_state_change` + `reject_duplicate_storage_proof_in_validator_mixed_block_without_state_change` — CLSAG privacy spend + SPoRA in one block still reject atomically on duplicate proof (`5c5975f`, `dd76d7f`). **M5.6++ (partial ✓):** legacy mixed `reject_mixed_tampered_storage_proof_root_without_state_change` + `reject_mixed_invalid_clsag_without_state_change` + `reject_mixed_after_partial_chain_without_state_change` (`0574d69`, `013bf47`). **M5.6+ (partial ✓):** `reject_validator_mixed_*_without_state_change` — tampered `storage_proof_root`, invalid coinbase, subquorum finality, invalid CLSAG, reject after partial chain preserves tip (`4b82b4c`, `c9db77e`).
- **Uniform ring-16 privacy policy.** **M5.31 (✓):** `tests/block_apply.rs` — `consensus_rejects_non_uniform_ring_sizes_across_inputs` + `apply_block_rejects_non_uniform_ring_sizes_across_inputs` reject heterogeneous ring sizes across inputs under production `uniform_ring_size = 16`. **M5.32 (✓):** `mfn-runtime` mempool `admit_rejects_non_uniform_ring_sizes` + `mfn-node/tests/mempool_integration.rs` — `mempool_rejects_non_uniform_ring_sizes_across_inputs` reject at mempool ingress before accept. **M5.31-docs/cli (✓):** `mfn-wallet/README.md` + `mfn-cli` help default ring-size 16. **M5.33 (✓):** `tests/apply_block_proptest.rs` — `prop_mixed_clsag_fee_and_storage_upload_treasury` (CLSAG fee + NEW storage upload same block treasury identity). **M5.35/M5.38 (✓):** `deep_mixed_clsag_fee_and_storage_upload_treasury_64` — 64-block same-block CLSAG fee + NEW storage upload treasury chain in default CI. **M5.39 (✓):** `deep_alternating_register_storage_treasury_8` — alternating bond-register + SPoRA treasury chain through epoch churn cap in default CI.
