//! Signed append-only checkpoint log (**F12**).
...
use crate::summary::{
    format_summary_diff, summaries_equal, summary_from_checkpoint_hex, weak_subjectivity_agrees,
...
/// Checkpoint log operation errors.
#[derive(Debug, Error, PartialEq, Eq)]
pub enum CheckpointLogError {
...
/// Parameters for signing a checkpoint log entry.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CheckpointLogSignParams {
    /// Weak-subjectivity summary to attest.
    pub summary: LightCheckpointSummary,
...
/// Result of verifying a checkpoint log.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
...
impl CheckpointLogEntry {
    ...
    pub fn verify(&self) -> Result<(), String> {
        ...
        let pk_bytes = crate::summary::decode_hex_fixed32(&self.signer_pk_hex, "signer_pk_hex")?;
        let sig_bytes = crate::summary::decode_hex_fixed64(&self.signature_hex, "signature_hex")?;
        ...
        if let Some(cp_hex) = self.checkpoint_hex.as_ref() {
            ...
            weak_subjectivity_agrees(&self.summary, cp_hex)
...
/// Build a signed checkpoint log entry.
pub fn checkpoint_log_sign(
    params: &CheckpointLogSignParams,
) -> Result<CheckpointLogEntry, CheckpointLogError> {
    ...
    let msg = entry.signing_bytes().map_err(CheckpointLogError::Usage)?;
    let sig = schnorr_sign_with(&msg, &kp, &mut rand_core::OsRng);
    entry.signature_hex = hex::encode(encode_schnorr_signature(&sig));
    Ok(entry)
}

/// Verify every JSONL line in `jsonl`.
pub fn checkpoint_log_verify_jsonl(
    jsonl: &str,
) -> Result<CheckpointLogVerifyReport, CheckpointLogError> {
    let reader = BufReader::new(Cursor::new(jsonl.as_bytes()));
    verify_reader(reader, "checkpoint log")
}

/// Cross-check a live weak-subjectivity summary against a signed checkpoint log JSONL.
pub fn cross_check_summary_against_checkpoint_log_jsonl(
    summary: &LightCheckpointSummary,
    jsonl: &str,
) -> Result<CheckpointLogCrossCheckReport, CheckpointLogError> {
    let reader = BufReader::new(Cursor::new(jsonl.as_bytes()));
    cross_check_reader(summary, reader, "checkpoint log")
}

/// Serialize one verified entry as a JSONL line (no trailing newline).
pub fn encode_checkpoint_log_entry(
    entry: &CheckpointLogEntry,
) -> Result<String, CheckpointLogError> {
    entry.verify().map_err(CheckpointLogError::Usage)?;
    serde_json::to_string(entry)
        .map_err(|e| CheckpointLogError::Usage(format!("encode entry: {e}")))
}

pub(crate) fn verify_reader<R: BufRead>(
    reader: R,
    label: &str,
) -> Result<CheckpointLogVerifyReport, CheckpointLogError> {
    ...
    for (line_no, line) in reader.lines().enumerate() {
        let line = line.map_err(|e| {
            CheckpointLogError::Usage(format!("read {label} line {}: {e}", line_no + 1))
        })?;
        ...
        let entry: CheckpointLogEntry = serde_json::from_str(trimmed).map_err(|e| {
            CheckpointLogError::Usage(format!("parse {label} line {}: {e}", line_no + 1))
        })?;
        entry
            .verify()
            .map_err(|e| CheckpointLogError::Usage(format!("{label} line {}: {e}", line_no + 1)))?;
    ...
    if valid_entries == 0 {
        return Err(CheckpointLogError::Usage(format!("{label} has no entries")));
    }
...
pub(crate) fn cross_check_reader<R: BufRead>(
    summary: &LightCheckpointSummary,
    reader: R,
    label: &str,
) -> Result<CheckpointLogCrossCheckReport, CheckpointLogError> {
    ...
    if entries_at_height > 0 {
        ...
        return Err(CheckpointLogError::Usage(format!(
            "checkpoint log disagrees with live sync at tip_height {}:\n{diff}",
            summary.tip_height
        )));
    }
    Err(CheckpointLogError::Usage(format!(
        "{label} has no attestation at tip_height {}",
        summary.tip_height
    )))
}

fn decode_seed_hex(hex_str: &str) -> Result<[u8; 32], CheckpointLogError> {
    crate::summary::decode_hex_fixed32(hex_str, "signer seed").map_err(CheckpointLogError::Usage)
}
...
mod tests {
    ...
    fn signed_entry(
        ...
        let params = CheckpointLogSignParams {
            summary,
            signer_id: signer_id.into(),
            signer_seed_hex: hex::encode(seed),
            checkpoint_hex: None,
        };
        checkpoint_log_sign(&params).expect("sign")
    }

    #[test]
    fn sign_and_verify_round_trip() {
        let entry = signed_entry([7u8; 32], "test-maintainer", sample_summary());
    ...
    fn cross_check_accepts_matching_entry() {
        ...
        let report = cross_check_summary_against_checkpoint_log_jsonl(&summary, &jsonl)
            .expect("cross-check");
    ...
    fn cross_check_rejects_disagreement_at_height() {
        ...
        let jsonl = encode_checkpoint_log_entry(&entry).expect("encode");
        let err = cross_check_summary_against_checkpoint_log_jsonl(&summary, &jsonl)
    ...
    fn tampered_signature_rejected() {
        let mut entry = signed_entry([8u8; 32], "test", sample_summary());
        entry.signature_hex = "00".repeat(64);
