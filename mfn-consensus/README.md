...
- **Validator finality evolution (M5 consensus)** (`tests/validator_finality_evolution.rs`, 16 tests) — pre-block `validator_root` quorum, liveness stats atomicity, bond/slashing/tx/storage_proof root mismatch reject without state change, invalid slash evidence reject without state change, reject preserves state, root movement on liveness/equivocation slash, equivocation during unbond delay, unbond delay preserves root, settlement moves successor root, exit-churn cap deferral.
