...
**Emission / treasury (M5.0 / M5.1):** default CI runs `mfn-consensus/tests/emission_simulation.rs` (100k-height curve + 10k empty blocks + 512-block storage-proof treasury ledger + 128-block CLSAG fee→treasury chain). Million-height / 2k fee-block checks are `#[ignore]`.
