...
foreach ($needle in @("verify_validity_proof_v1", "VALIDITY_PROOF_V1_TAG", "0x14", "phase 4b.1")) {
...
if (-not (Select-String -LiteralPath $Consensus -Pattern "build_stark_winterfell_validity_proof" -Quiet)) {
    throw "validity-proof-rehearsal-smoke: validity_proof.rs missing winterfell builder"
}
...
Write-Host "  witness=apply_block_replay + stark_digest_stub + stark_winterfell (phase 4b.1)"
