...
- **Producer treasury settlement (M5 economics, 19 tests)** (`tests/producer_treasury_settlement.rs`) — 90/10 fee split, coinbase = emission + producer share + storage rewards (+ PPB bonus), treasury drain vs backstop, invalid/overpaid coinbase reject without state change, bond/slash/liveness/fee/proof closed loops, five-path treasury composition, PPB carry-over, and PPB-augmented proof-drain composition.
