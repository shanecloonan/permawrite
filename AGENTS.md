...
| **7** Testnet launch | **B-31 probe** + **B-30** | *Idle* | **B-40** P2P bind repair after B-15; **B-22** needs human signer seed; then TL-9 | `launch-go-no-go` |
...
| planning | 1 | **B-34:** restart CI `#29711375639` (all jobs queued, empty steps) via docs push without `[skip ci]` | **Done** (this land) |
| planning | 2+7 | **B-31:** use ROADMAP work package before TL-9 (RPC/faucet/TLS verify) | **Done** (probe landed; P2P FAIL → B-40) |
| 7 | 2+3+human | **B-40:** after B-15 lock — fix VPS `vps-bind.env` P2P to `0.0.0.0:1900x`, restart mfnd-* only, confirm outside dial to seeds | **Blocked** (B-15 §6) |
...
| B-40 | Repair public P2P binds (`0.0.0.0:1900x`) on Hetzner | 7+2 | After B-15; seeds currently loopback-only (B-31 FAIL) |
...
| B-31 | Live RPC/faucet threat posture verify | 2+7 | **Probed** — evidence `b31-threat-posture-20260720.md`; RPC PASS; P2P FAIL → B-40; close after B-40+B-26 |
...
| B-34 | CI queue/stall watch + cancel/re-dispatch | 1 | Restarting `#29711375639` this land; protocol in ROADMAP |
...
5. **2026-07-19 — lane 1 — B-29 Nightly WS tip fix** (`5dc3aa8`): empty-UTXO light-sync wiped `scan_height`; fund-wallet `--reset-trusted-summary`. Close = Nightly GREEN.
6. **2026-07-19 — planning — B-29 work package (Nightly ≠ B-15)** (`583bf11`): JOIN does not close B-29. Docs-only `[skip ci]`.
7. **2026-07-19 — planning — L4 invite gates B-26…B-31** (`8254c51`): Nightly RED → B-29. Docs-only `[skip ci]`.
8. **2026-07-19 — lane 5 — B-16 privacy-doc sync** (`49d28f9`): JOIN/TESTNET/PRIVACY/INVITE/OPERATORS + wallet/WASM. Docs-only `[skip ci]`.
9. **2026-07-19 — planning — Phase 1 permanence playbook** (`55c4abc`): **B-25** before Tier 2/Path B. Docs-only `[skip ci]`.
10. **2026-07-19 — lane 3 — B-15 checkpoint light-scan** (`02c8df8` / `73abf77`): JOIN/`fund-wallet-http`.
