...
**CI gate (2026-07-23):** Landing **B-197** WASM/CLI F7 faucet dual-send message parity (full CI). B-210 matrix GREEN on `#30028287920` (dispatch may cancel). **B-189** CI `#29946719956` GREEN. Lane4 may resume **B-211** after this tip. Strategic path: L4 → **B-40** → **B-13a** → **B-25**.
...
| **4** Protocol | **B-210** (`6339fe65`); **B-209**/**B-208** (CI `#30023674882` GREEN); **B-207** | **B-211** thirteenth→dual settle (claim base: `6339fe65`) — resume after B-197 tip CI | After land: thirteenth asymmetric; live **B-32** needs 2nd host | Lane 1 CI |
| **5** Privacy | **B-197** WASM/CLI F7 faucet dual-send message parity (this commit); **B-189** (`ecc565a1`; CI `#29946719956` GREEN); **B-186** | *Idle* | After B-25: **B-35** / **B-37** / **B-19** | Doc-accuracy duty |
...
| B-197 | WASM/CLI F7 faucet dual-send error-message parity (elevates B-189) | 5 | **Landed** (this commit) — WASM + CLI `map_wallet_build_err` faucet dual-send wording; privacy-floor smoke; full CI |
...
1. **2026-07-23 — lane 5 — B-197 WASM/CLI F7 faucet dual-send message parity** (this commit): WASM transfer/upload one-input rejects mention `faucet dual-send`; CLI `map_wallet_build_err` rewrites `TxInputCountBelowMinimum` to B-189 preflight text; PRIVACY + PRIVACY_HARDENING honesty; privacy-floor smoke needles. Local wasm-full + CLI units PASS. B-210 matrix GREEN on `#30028287920`. Full CI (no skip). §6 CI-window **Done**. Next: after B-25 **B-35**/**B-37**/**B-19**. *Observed (not staged):* lane-4 `apply_block_proposals.rs`.
