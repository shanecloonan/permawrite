...
**Emission / treasury (M5.0 / M5.1 / M5.1+):** default CI runs `mfn-consensus/tests/emission_simulation.rs` (100k-height curve + 10k empty blocks + 512-block storage-proof ledger + 128-block CLSAG fee chain + 48-block mixed fee+proof blocks). Million-height / 2k fee / 384 mixed checks are `#[ignore]`.
