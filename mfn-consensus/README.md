...
- **apply_block proptest (M5.2–M5.36)** (`tests/apply_block_proptest.rs`, 32 default + 4 nightly `#[ignore]`) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain) + bond inflow/random fee/SPoRA treasury identity + **M5.33** mixed CLSAG fee + NEW storage upload same-block treasury (`prop_mixed_clsag_fee_and_storage_upload_treasury`) + **M5.35** 64-block deep upload treasury chain (`deep_mixed_clsag_fee_and_storage_upload_treasury_64`) + **M5.36** 64-block deep CLSAG+SPoRA treasury chain (`deep_mixed_clsag_fee_and_storage_proof_treasury_64`) + validator combined-inflow, equivocation combined-inflow, and random-schedule treasury identity with equivocation and no-equivocation variants.
