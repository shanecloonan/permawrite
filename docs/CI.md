...
**RC Validation After CI (M2.4.77, M2.5.18, M2.5.31):** Green **push** CI on `main` auto-dispatches **Nightly** via the `dispatch-nightly-rc` job in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) with `ref: main` and `inputs.checkout_sha` set to the exact passing commit (**M2.4.83** — GitHub rejects raw commit SHAs as dispatch refs). [`.github/workflows/rc-validation-after-ci.yml`](../.github/workflows/rc-validation-after-ci.yml) provides a **`workflow_run` backup** (**M2.5.31**): when CI completes successfully on `main` but inline `dispatch-nightly-rc` did not succeed, it dispatches Nightly at the green commit; if inline dispatch already succeeded, the backup is a no-op. Manual operator re-dispatch remains via `workflow_dispatch` + optional `ci_head_sha`. Manual **Nightly** dispatch also via `dispatch-rc-workflows.ps1 -Nightly` or Actions UI.

**Linux Soak Audit (M2.4.74, B-05):** [`.github/workflows/linux-soak-audit.yml`](../.github/workflows/linux-soak-audit.yml) — manual `workflow_dispatch` or **auto-dispatch** via `dispatch-linux-soak-rc` in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml) when no `soak-restart-linux-30s-slot-*.txt` is archived on `main`. Pre-builds **`mfnd` + `mfn-cli`** before `soak.sh` → `start-all.sh --no-build` (`query_tip_height` needs `mfn-cli` when nc JSON-RPC is empty). 35 min 30s-slot soak with observer restart; artifact upload; PASS transcripts auto-commit with `[skip ci]`. Import fallback: [`import-linux-soak-artifact.ps1`](../scripts/public-devnet-v1/import-linux-soak-artifact.ps1) / [`import-linux-soak-artifact.sh`](../scripts/public-devnet-v1/import-linux-soak-artifact.sh).
...
**Agent board UTF-8 guard (M2.5.26, M2.5.27):** the same scripts also fail closed on UTF-16/null-byte corruption in `AGENTS.md`, `docs/AGENTS_LEDGER.md`, the `docs/AGENTS.md` / `3agent.md` pointer stubs, and `docs/STORAGE_ACCESSIBILITY.md`. Since the 2026-07-19 board consolidation, `AGENTS.md` is the only live coordination surface and `docs/AGENTS_LEDGER.md` is its append-only history.
