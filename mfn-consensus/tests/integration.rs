...
/// Two consecutive legacy mixed blocks (**M5.5+**): each block carries CLSAG fee +
/// SPoRA proof (no coinbase, no BLS); treasury ledger matches fee-credit /
/// storage-drain model across both heights.
#[test]
fn legacy_mixed_two_block_treasury_ledger_identity() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_consensus::{
        apply_block, apply_genesis, build_genesis, build_unsealed_header, seal_block, ApplyOutcome,
        ConsensusParams, GenesisConfig, GenesisOutput, TransactionWire, DEFAULT_EMISSION_PARAMS,
    ...
    for height in 1..=2u32 {
        ...
        st = match apply_block(&st, &block) {
            ApplyOutcome::Ok { state, .. } => state,
            ApplyOutcome::Err { errors, .. } => panic!("height {height} rejected: {errors:?}"),
        };
        model_treasury =
            treasury_after_block(model_treasury, u128::from(fee), 1, &DEFAULT_EMISSION_PARAMS);
        assert_eq!(
            st.treasury, model_treasury,
            "treasury mismatch at height {height}"
        );
        assert_eq!(st.height, Some(height));
        assert_eq!(st.storage[&commit_hash].last_proven_height, height);
        assert_eq!(st.spent_key_images.len(), height as usize);
    }
}
...
/// After one valid legacy mixed block, post-seal tamper of `storage_proof_root`
/// at height 2 must reject with unchanged tip (**M5.5+** integration).
#[test]
fn reject_legacy_mixed_tampered_storage_proof_root_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_consensus::{
        apply_block, apply_genesis, block_id, build_genesis, build_unsealed_header,
        encode_chain_checkpoint, seal_block, ApplyOutcome, BlockError, ChainCheckpoint,
    ...
    let fee1 = 100_000u64;
    let (tx1, spend) = initial_spend.sign_self_transfer(fee1, 1);
    st = match apply_block(&st, &seal_legacy_mixed(&st, 1, tx1, &built, &payload)) {
        ApplyOutcome::Ok { state, .. } => state,
        ApplyOutcome::Err { errors, .. } => panic!("block1 rejected: {errors:?}"),
    };
    assert_eq!(st.height, Some(1));
    ...
    let mut bad_block = seal_legacy_mixed(&st, 2, tx2, &built, &payload);
    bad_block.header.storage_proof_root[0] ^= 0xff;
...
/// After one valid legacy mixed block, duplicate SPoRA proofs at height 2
/// must reject with unchanged tip (**M5.5+** integration).
#[test]
fn reject_legacy_mixed_duplicate_storage_proof_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_consensus::{
        apply_block, apply_genesis, block_id, build_genesis, build_unsealed_header,
        encode_chain_checkpoint, seal_block, ApplyOutcome, BlockError, ChainCheckpoint,
    ...
    fn seal_legacy_mixed_duplicate_proof(
        st: &mfn_consensus::ChainState,
        height: u32,
        tx: TransactionWire,
        built: &mfn_storage::BuiltCommitment,
    ...
    let fee2 = 50_000u64;
    let (tx2, _) = spend.sign_self_transfer(fee2, 2);
    let bad_block = seal_legacy_mixed_duplicate_proof(&st, 2, tx2, &built, &payload);
...
/// Two consecutive validator-mode mixed blocks (**M5.6+**): each block carries
/// CLSAG fee + coinbase + BLS quorum + SPoRA proof; treasury ledger matches
/// the fee-credit / storage-drain model across both heights.
#[test]
fn validator_mixed_two_block_treasury_ledger_identity() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_bls::bls_keygen_from_seed;
    use mfn_consensus::{
        apply_block, apply_genesis, build_genesis, build_unsealed_header, cast_vote,
    ...
    let (v0, s0, _) = mk_validator(0, 100);
    let (v1, s1, _) = mk_validator(1, 100);
    let (v2, s2, _) = mk_validator(2, 100);
    let validators = vec![v0.clone(), v1.clone(), v2.clone()];
    let total_stake: u64 = validators.iter().map(|v| v.stake).sum();
...
/// After one valid validator mixed block, a tampered CLSAG fee tx at height 2
/// must reject and leave tip state byte-for-byte unchanged (**M5.6+** integration).
#[test]
fn reject_validator_mixed_tampered_clsag_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_bls::bls_keygen_from_seed;
    use mfn_consensus::{
...
/// After one valid validator mixed block, post-seal tamper of `storage_proof_root`
/// at height 2 must reject with unchanged tip (**M5.6+** integration).
#[test]
fn reject_validator_mixed_tampered_storage_proof_root_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_bls::bls_keygen_from_seed;
    use mfn_consensus::{
        apply_block, apply_genesis, block_id, build_genesis, build_unsealed_header, cast_vote,
...
/// After one valid validator mixed block, sub-quorum BLS finality at height 2
/// (two of three validators) must reject with unchanged tip (**M5.6+**).
#[test]
fn reject_validator_mixed_subquorum_finality_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_bls::bls_keygen_from_seed;
    use mfn_consensus::{
        apply_block, apply_genesis, block_id, build_genesis, build_unsealed_header, cast_vote,
    ...
    match apply_block(&st, &bad_block) {
        ApplyOutcome::Err { errors, .. } => {
            assert!(
                ...
                "expected FinalityInvalid, got {errors:?}"
            );
        }
        ApplyOutcome::Ok { .. } => panic!("sub-quorum finality must reject"),
    }
    assert_eq!(
        encode_chain_checkpoint(&ChainCheckpoint {
            genesis_id,
...
/// After one valid validator mixed block, duplicate SPoRA proofs at height 2
/// must reject with unchanged tip (**M5.6+** integration).
#[test]
fn reject_validator_mixed_duplicate_storage_proof_after_one_block_without_state_change() {
    use curve25519_dalek::edwards::EdwardsPoint;
    use mfn_bls::bls_keygen_from_seed;
    use mfn_consensus::{
        apply_block, apply_genesis, block_id, build_genesis, build_unsealed_header, cast_vote,
