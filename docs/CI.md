...
**RC Validation After CI (M2.4.77):** [`.github/workflows/rc-validation-after-ci.yml`](../.github/workflows/rc-validation-after-ci.yml) listens for successful **CI** workflow runs on `main` (`push` or `workflow_dispatch`) and auto-dispatches **Nightly** on the exact passing commit (**M2.4.82** also accepts manual CI re-runs). Manual **Nightly** dispatch remains available via `dispatch-rc-workflows.ps1 -Nightly` or Actions UI.
