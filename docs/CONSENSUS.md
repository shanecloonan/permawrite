...
- **`tests/validator_finality_evolution.rs` (M5 consensus, `a97242a` / `267658d` / `4715544` / `b50662f` / `7452127` / `ce851f8` / `e10b249` / `ebfa7b0` / `9556053`)** — `finality_quorum_uses_pre_block_validator_set`, `validator_root_is_pre_block_in_validator_mode`, liveness bitmap + stats evolve atomically on accept, `liveness_stats_unchanged_when_block_rejected`, `rejected_block_leaves_state_unchanged`, `bond_root_mismatch_rejects_without_state_change`, **`slashing_root_mismatch_rejects_without_state_change`**, **`tx_root_mismatch_rejects_without_state_change`**, **`storage_proof_root_mismatch_rejects_without_state_change`**, **`claims_root_mismatch_rejects_without_state_change`**, **`storage_root_mismatch_rejects_without_state_change`**, **`utxo_root_mismatch_rejects_without_state_change`**, **`invalid_slash_evidence_rejects_without_state_change`**, **`duplicate_slash_evidence_rejects_without_state_change`**, `validator_root_moves_on_liveness_slash`, `equivocation_slash_moves_successor_validator_root`, **`equivocation_during_unbond_delay_still_zeros_stake`**, **`unbond_request_preserves_validator_root_in_delay_window`**, **`validator_root_moves_on_unbond_settlement`**, **`exit_churn_cap_defers_third_unbond_settlement`**, **`exit_churn_cap_resets_at_epoch_boundary`**, **`entry_churn_cap_rejects_third_register_without_state_change`**, **`entry_churn_cap_allows_two_registers_and_moves_validator_root`**, **`entry_churn_cap_resets_at_epoch_boundary`**, plus bond-op admission rejects for duplicate VRF registration, duplicate unbond enqueue, duplicate unbond after an already pending exit, below-minimum stake, same-block register-then-unbond, forged unbond, unknown validator, zombie validator, same-block duplicate VRF, and all-or-nothing treasury preservation on rejected bond ops — all without state mutation.
- **`tests/apply_block_proptest.rs` (Agent 1 rollback, `5c5975f` / `dd76d7f` / `4b82b4c` / `0574d69` / `013bf47` / `c9db77e`)** — `reject_duplicate_storage_proof_in_mixed_block_without_state_change` (legacy CLSAG + SPoRA same block); `reject_duplicate_storage_proof_in_validator_mixed_block_without_state_change` (validator quorum + coinbase + CLSAG + SPoRA); **M5.6++ (legacy)** `reject_mixed_tampered_storage_proof_root_without_state_change`, `reject_mixed_invalid_clsag_without_state_change`, **`reject_mixed_after_partial_chain_without_state_change`**; **M5.6+ (validator)** `reject_validator_mixed_tampered_storage_proof_root_without_state_change`, `reject_validator_mixed_invalid_coinbase_without_state_change`, `reject_validator_mixed_subquorum_finality_without_state_change`, `reject_validator_mixed_invalid_clsag_without_state_change`, **`reject_validator_mixed_after_partial_chain_without_state_change`** — each assert state unchanged on reject; **`prop_reject_after_partial_chain`** proptest for same invariant.
- **`tests/integration.rs` (Agent 1 legacy mixed integration, `ba04799` / `6422d1a` / `d99d424` / `71350e4` / `925561a`)** — `legacy_mixed_clsag_fee_and_storage_proof_at_genesis_plus_block1` proves legacy CLSAG fee + SPoRA forward-apply with treasury identity in a real block flow; `legacy_mixed_two_block_treasury_ledger_identity` carries the same privacy-fee + SPoRA treasury credit/drain identity across two blocks; `reject_legacy_mixed_tampered_clsag_after_one_block_without_state_change` proves a tampered legacy mixed CLSAG after prior valid history rejects without moving the tip; `reject_legacy_mixed_tampered_storage_proof_root_after_one_block_without_state_change` proves post-seal `storage_proof_root` tamper after prior valid legacy mixed history rejects without moving the tip; `reject_legacy_mixed_duplicate_storage_proof_after_one_block_without_state_change` proves duplicate SPoRA proof replay after prior valid legacy mixed history rejects without moving the tip.
- **`tests/integration.rs` (Agent 1 validator mixed integration, `30b35c0` / `2063202` / `56a53f3` / `302f3b7` / `e07d526` / `a09b48c` / `925561a`)** — `validator_mixed_clsag_fee_and_storage_proof_at_genesis_plus_block1` proves validator-mode BLS finality + coinbase + CLSAG fee + SPoRA forward-apply; `validator_mixed_two_block_treasury_ledger_identity` carries that invariant across two blocks and checks treasury credit/drain identity; `reject_validator_mixed_tampered_clsag_after_one_block_without_state_change` proves a tampered validator-mixed CLSAG after a prior valid block rejects without moving the tip; `reject_validator_mixed_tampered_storage_proof_root_after_one_block_without_state_change` proves post-seal `storage_proof_root` tamper after prior valid validator-mixed history rejects without moving the tip; `reject_validator_mixed_invalid_coinbase_after_one_block_without_state_change` proves underpaid validator coinbase after prior valid mixed history rejects without moving the tip; `reject_validator_mixed_subquorum_finality_after_one_block_without_state_change` proves sub-quorum BLS finality after prior valid mixed history rejects without moving the tip; `reject_validator_mixed_duplicate_storage_proof_after_one_block_without_state_change` proves duplicate SPoRA proof replay after prior valid validator mixed history rejects without moving the tip.
