...
- **Integration** (`tests/integration.rs`, 16 tests) — multi-block flows: genesis → block1 → block2 with privacy tx, storage upload, slashing; full `unbond_lifecycle` with 3 validators, BLS finality, request → delay → settle, equivocation-during-delay still slashes, exit-churn cap spills across blocks; post-sign header tamper rejects for `validator_root`, `slashing_root`, `storage_proof_root`; validator mixed CLSAG fee + SPoRA forward-apply and two-block treasury ledger identity.
...
- **apply_block proptest (M5.2–M5.7)** (`tests/apply_block_proptest.rs`, 28 tests) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain) + bond inflow/random fee/SPoRA treasury identity.
