#!/usr/bin/env bash
# Lane 7: plan-only launch-status v10 schema rehearsal.
set -euo pipefail
...
Validates launch-status.v10 JSON schema + checkpoint_log + execution_checklist (no VPS required).
EOF
}
...
assert doc.get("schema_version") == "launch-status.v10", doc.get("schema_version")
...
assert fp.get("validity_proof_phase") == "4b.1", fp.get("validity_proof_phase")
assert fp.get("stark_backend") == "winterfell", fp.get("stark_backend")
assert fp.get("p2p_tag_validity") == "0x14", fp.get("p2p_tag_validity")
print("launch-status-rehearsal-smoke: plan")
print("  schema=launch-status.v10")
