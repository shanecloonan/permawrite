...
- **Validator finality evolution (M5 consensus)** (`tests/validator_finality_evolution.rs`, 29 tests) — pre-block `validator_root` quorum, liveness stats atomicity, bond/slashing/tx/storage_proof root mismatch reject without state change, invalid/duplicate slash evidence reject without state change, reject preserves state, root movement on liveness/equivocation slash, equivocation during unbond delay, unbond delay preserves root, settlement moves successor root, exit-churn cap deferral/reset, entry-churn cap reject/accept/reset, and bond-op admission rejects for duplicate VRF, duplicate unbond, below-minimum stake, same-block register-then-unbond, forged/unknown/zombie unbond, and same-block duplicate VRF.
