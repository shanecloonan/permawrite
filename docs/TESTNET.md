...
A participant rehearsal is not considered successful until the final line says `participant-rehearsal: PASS ... support_bundle=<dir>` and the helper prints `participant-rehearsal: evidence_log=<file>`. Use `-EvidenceDir` / `--evidence-dir` to stage `<evidence-dir>/participant-rehearsal.log` and `<evidence-dir>/support-bundle` together for release audit. Archive that evidence log and support-bundle directory with launch evidence; a timeout, daemon-death diagnostic, hash mismatch, missing proof, missing evidence log, or missing support bundle is still a public-invite blocker.

When building release-candidate launch evidence, pass the archived participant rehearsal evidence to `release-audit-packet.ps1` / `release-audit-packet.sh`. After a local smoke run, `-ParticipantEvidenceDir ./participant-rehearsal-smoke/evidence` / `--participant-evidence-dir ./participant-rehearsal-smoke/evidence` resolves the co-located `participant-rehearsal.log` and `support-bundle/` staged by the smoke wrapper; pass `-ParticipantRehearsalLog` / `--participant-rehearsal-log` and `-ParticipantSupportBundle` / `--participant-support-bundle` separately when evidence is not co-located. The audit packet validates the PASS line, restored SHA-256 shape, the log's `support_bundle` reference against the provided bundle directory, support-bundle `manifest.json`, read-only marker, matching commitment hash, and core node/upload/proof captures without rerunning the live rehearsal.
...
| **Observer** | `serve` (no produce flags) | Syncs blocks/txs, exposes JSON-RPC; periodic catch-up dials to durable peers (**M2.4.62**); no validator env required. |

The public helper mesh runs one hub producer plus two committee voters and one observer. With `expected_proposers_per_slot: 1.5`, validator 0 is ineligible at slot 1; the hub scans forward through bounded slots within each producer tick instead of waiting a full slot interval, so genesis does not stall. `--produce` nodes do not run periodic committee catch-up dials (**M2.3.29** / **M2.4.61**); only `--committee-vote` followers pull. Inbound ephemeral TCP source ports are tracked for live fan-out but excluded from committee catch-up, boot reconnect, and `peers.json` persistence (**M2.4.62**). Proposal fan-out reads committee `VoteV1` replies on both fresh dials and registered sessions (**M2.3.30**). Inbound P2P blocks must be exactly `tip_height + 1` (**M2.3.31**); stale or gap frames reject before `apply` (catch-up aborts stay clean).
...
- **Catch-up:** Outbound dials pull missing blocks when the remote tip is ahead; handshake height uses the live chain tip (**M2.3.24+**). Periodic committee catch-up and gap-triggered recovery use the same available peer set, skip quarantined peers before cap accounting, skip the node's own P2P listen address, and honor `max_outbound_peers`; self-skips log `mfnd_p2p_self_dial_skip peer=...`, capped committee intervals log `mfnd_p2p_catchup_cap_reached count=... cap=...`, and capped gap-triggered recovery logs `mfnd_p2p_gap_catchup_cap_reached count=... cap=...`.
...
- **Peer quarantine:** Repeated outbound dial, fan-out, or catch-up failures temporarily quarantine a peer in memory; while quarantined, reconnect, committee catch-up, gap catch-up, and fan-out skip that address. Successful handshakes or pushes clear the penalty. Transient boot-dial connect failures quarantine stale public seeds without deleting them from `peers.json`; only genesis-mismatch failures durably drop saved peers. Operators can watch for `mfnd_p2p_peer_quarantine peer=...` in logs.
...
- `scripts/public-devnet-v1/health-check.{sh,ps1}` and `soak.{sh,ps1}` syntax checks — default public-devnet health requires hub, both voters, and observer RPC endpoints to be discoverable; `soak --restart-observer-once` / `-RestartObserverOnce` records observer kill/restart catch-up evidence via `soak: RESTART`; set `MFN_HEALTH_REQUIRE_ALL_ROLES=0` only for intentional partial-mesh diagnostics.
