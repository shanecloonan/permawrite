...
[![Tests](https://img.shields.io/badge/tests-602_passing-brightgreen)](#status)
...
**Pre-network.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` binary exercises checkpoint persistence, optional JSON genesis (`--genesis`), solo **`step`** (mempool drain + coinbase + produce / apply / save, with **`--blocks N`** and optional **`--checkpoint-each`** for per-block checkpoints), env-gated validator seeds, and **`serve`** (blocking TCP, one request line per connection: **`get_tip`** and **`submit_tx`** toward the in-memory mempool — not full JSON-RPC yet); there is no P2P network, wallet-facing JSON-RPC, or public testnet yet. As of the latest commit:
...
| **Total** | | **602** | Zero `unsafe`. Zero clippy warnings. Two ignored placeholders/doc examples. |
...
# Run the full workspace test suite (602 passing tests, ~6 min on a modern machine)
