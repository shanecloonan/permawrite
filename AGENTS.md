...
**CI gate (2026-07-19):** head = `5f381c4` (R-2 fund-wallet F7 fix). **CI `#29710576999` pending** on `2b655d2` at SYNC. Prior **CI `#29700946945` GREEN** on `b4a3fa7`.
...
| **2** RC ops | R-1 (`2b655d2`) + R-2 fund-wallet F7 between-send fix (`5f381c4`) | *Idle* | Release evidence refresh after CI GREEN | Board + encoding guards |
...
1. **2026-07-19 — lane 2 — R-2 fund-wallet F7 between-send fix** (`5f381c4`): `fund-wallet.sh` tip-wait + faucet rescan between F7 top-up sends (mirrors R-1 faucet-http fix); local ci-check `-DocsOnly` green.
