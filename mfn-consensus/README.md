...
- **Producer treasury settlement (M5 economics, 16 tests)** (`tests/producer_treasury_settlement.rs`) — 90/10 fee split, coinbase = emission + producer share + storage rewards (+ PPB bonus), treasury drain vs backstop, invalid/overpaid coinbase reject without state change, bond/slash/liveness/fee/proof closed loops, PPB carry-over.
- **apply_block proptest (M5.2–M5.6+)** (`tests/apply_block_proptest.rs`, 27 tests) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain).
