...
**Tests:** 380 passing (151 unit + 229 integration across six `tests/` targets; 14 nightly `#[ignore]`) &nbsp;·&nbsp; **`unsafe`:** forbidden &nbsp;·&nbsp; **Clippy:** `-D warnings` (see `scripts/ci-check`)
...
- **Emission simulation (M5.0–M5.40)** (`tests/emission_simulation.rs`, 24 default + 13 nightly `#[ignore]`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains (**96-block** validator CLSAG-only and **64-block** validator mixed CLSAG+SPoRA in CI, **M5.35** / **M5.34/B-03**; **384-block** legacy mixed fee+proof in CI, **M5.39**; **64-block** combined-inflow + PPB + equivocation-PPB combined-inflow in CI, **M5.40**), liveness-slash, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
