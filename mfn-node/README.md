...
| [`network`](src/network/mod.rs) | **M2.3.0–M2.3.6** P2P scaffold: [`NetworkConfig`](src/network/mod.rs); **M2.3.1** [`frame`](src/network/frame.rs) + **M2.3.5** [`PingV1`](src/network/frame.rs) / [`PongV1`](src/network/frame.rs); **M2.3.2** [`handshake`](src/network/handshake.rs) + **M2.3.4** / **M2.3.5** dial helpers; **M2.3.3** **`mfnd serve --p2p-listen`** (hello + listener pong, 30s read/write timeouts on accepted sockets); **M2.3.6** **`mfnd serve --p2p-dial`** (outbound full peer handshake). |
...
- **Unit (`network::*`, M2.3.0–M2.3.5)**: 19 tests — [`NetworkConfig::default`](src/network/mod.rs); [`network::frame`](src/network/frame.rs) length-prefix + [`HelloV1`](src/network/frame.rs) + **M2.3.5** [`PingV1`](src/network/frame.rs) / [`PongV1`](src/network/frame.rs); [`network::handshake`](src/network/handshake.rs) TCP hello, `tcp_connect_hello_v1_handshake`, `tcp_connect_peer_v1_handshake`, foreign-genesis rejection, invalid first frame.
...
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.3 + M2.3.4 + M2.3.5 + M2.3.6)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails, **`step --blocks 3`** reaches height 3 in one invocation, **`step --blocks 2 --checkpoint-each`** emits two `step_checkpoint` lines, **`mfnd_step_block_log_passes_validated_read`** (`read_block_log_validated` after one `step`), **`serve --rpc-listen 127.0.0.1:0`** + TCP `get_tip` (`mfnd_serve_get_tip_over_tcp`) + **`mfnd_serve_p2p_hello_handshake_over_tcp`** (`--p2p-listen`, `get_tip` genesis hex → [`tcp_connect_peer_v1_handshake`](src/network/handshake.rs)) + **`mfnd_serve_p2p_dial_hits_peer_listener`** (peer B `--p2p-dial` to peer A's `--p2p-listen`, reads `mfnd_p2p_dial_ok=`) + **`mfnd_serve_get_tip_jsonrpc_echoes_id`** + **`mfnd_serve_get_mempool_over_tcp_empty`** + **`mfnd_serve_get_mempool_lists_tx_after_submit`** (non-empty pool, **`get_mempool_tx`** round-trip, **`remove_mempool_tx`**, empty **`get_mempool`**) + **`mfnd_serve_clear_mempool_after_submit`** + **`mfnd_serve_get_checkpoint_round_trips_over_tcp_after_step`** + **`mfnd_serve_save_checkpoint_creates_checkpoint_file`** + **`mfnd_serve_list_methods_over_tcp`** + **`mfnd_serve_authorship_discovery_rpcs_over_tcp`**, **`mfnd_serve_get_block_over_tcp_after_step`** (also **`get_block_header`** vs `read_block_log_validated`), **`serve`** `submit_tx` negative paths (`mfnd_serve_submit_tx_rejects_*`, **`mfnd_serve_submit_tx_array_params_rejects_bad_hex`**), **`mfnd_step_writes_block_log_then_serve_submit_tx_admits_transfer`** (synth-decoy genesis + `chain.blocks` + signed transfer admitted as `Fresh`).
...
- `#![forbid(unsafe_code)]`.
- No background threads in core chain/mempool/store APIs; no clocks, no async runtime. **`mfnd serve`** runs JSON-RPC on a blocking **`std::net::TcpListener`** on the main thread (one client at a time). **M2.3.3** `serve --p2p-listen` adds one background **`std::thread`** for P2P accepts (**M2.3.5**: hello + ping/pong; 30s per-socket I/O timeouts). **M2.3.6** `serve --p2p-dial` adds another for outbound dials. Full gossip / libp2p / `tokio` remain future milestones.
