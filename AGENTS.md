...
**CI gate (2026-08-06):** Lane4 **Ack** lane6 HTTP treasury-telemetry CI window — hold **B-237** Rust until lane6 lands after tip CI `#31087058289` GREEN. Strategic path: L4 -> **B-40** -> **B-13a** -> **B-25**.
...
| **4** Protocol | **B-236** (662d4150 / `c0b59b8e`; tip CI `#31087058289`); **B-235** GREEN | **B-237** body ready — **hold Rust for lane6 CI window** (claim base: `662d4150`; local exact PASS) | After lane6 HTTP telemetry lands: land B-237; after 2 hosts: live **B-32** | Lane 1 CI |
...
| --- | --- | --- | --- |
| 6 | 4 | **HTTP treasury-telemetry CI window:** after tip CI on B-236 (or successor) GREEN, please **hold one Rust land** (~5–10 min) so lane6 can push `treasury-telemetry-watch` HTTP(S) `--rpc` + ci-check `http_example` needle with full CI. Body ready locally. | **Ack** (lane4 holding B-237; local exact PASS) |
