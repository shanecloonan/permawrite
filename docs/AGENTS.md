...
AGENTS.md (master)  <--------- claim / status / backlog
       |
       ╬ô├╢┬ú------ docs/AGENTS.md (this file) ΓÇö per-lane detail
       ╬ô├╢┬ú------ docs/TESTNET_CHECKLIST.md ΓÇö RC mirror for lanes 1ΓÇô3
       ╬ô├╢├╢------ 3agent.md ΓÇö alias pointer to lanes 1ΓÇô3
```
...
| --- | --- | --- | --- |
| Chat (start + end of unit) | yes | yes | yes |
| `AGENTS.md` current board | yes | yes | yes |
| This file ΓÇö lane section | yes | ΓÇö | yes |
| `3agent.md` (lanes 1ΓÇô3 only) | yes | yes | yes |
...
- [x] M2.4.89 Windows mirror ΓÇö `ci-check.ps1` `--test-threads=2` (`8e6b3c1`).
- [x] M7.10 push-all-chunks (`c1e0373` on `main`).`n- [x] M7.11 STORAGE_ACCESSIBILITY section 0 (this commit).
...
- [x] M2.5.26 UTF-8 board guard in validate-workflow-encoding (this commit).`n
### Next
...
**Owns:** Participant/observer rehearsal smokes, faucet/demo scripts, operator onboarding polish, M7.10/M7.11 UX docs.
...
- [x] **M5.48** - emission deep-sim tier closure; 2048-block CLSAG (~13m) + 100k empty `apply_block` (~7m) stay nightly-only (`df80b97`).
