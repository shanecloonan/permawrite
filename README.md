...
[![Tests](https://img.shields.io/badge/tests-598_passing-brightgreen)](#status)
...
**Pre-network.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` binary exercises checkpoint persistence, optional JSON genesis (`--genesis`), solo **`step`** (mempool drain + coinbase + produce / apply / save, with **`--blocks N`** and optional **`--checkpoint-each`** for per-block checkpoints), and env-gated validator seeds; there is no P2P network, JSON-RPC surface, or public testnet yet. As of the latest commit:
...
| **Total** | | **598** | Zero `unsafe`. Zero clippy warnings. Two ignored placeholders/doc examples. |
...
# Run the full workspace test suite (598 passing tests, ~6 min on a modern machine)
