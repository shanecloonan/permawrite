...
- **Integration** (`tests/integration.rs`, 14 tests) — multi-block flows: genesis → block1 → block2 with privacy tx, storage upload, slashing; full `unbond_lifecycle` with 3 validators, BLS finality, request → delay → settle, equivocation-during-delay still slashes, exit-churn cap spills across blocks; post-sign header tamper rejects for `validator_root`, `slashing_root`, `storage_proof_root`.
- **Block apply** (`tests/block_apply.rs`, 57 tests) — root mismatches; SPoRA binding (emit order, tampered root before payout, dual-proof payout, body tamper atomic reject, provenance + treasury on accept, unknown/wrong-chunk/duplicate rejects).
