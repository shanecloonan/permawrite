...
- **`tests/validator_finality_evolution.rs` (M5 consensus, `a97242a` / `267658d`)** — `finality_quorum_uses_pre_block_validator_set`, `validator_root_is_pre_block_in_validator_mode`, liveness bitmap + stats evolve atomically on accept, `liveness_stats_unchanged_when_block_rejected`, `rejected_block_leaves_state_unchanged`, `bond_root_mismatch_rejects_without_state_change`, **`slashing_root_mismatch_rejects_without_state_change`**, `validator_root_moves_on_liveness_slash`, `equivocation_slash_moves_successor_validator_root`, **`unbond_request_preserves_validator_root_in_delay_window`**, **`validator_root_moves_on_unbond_settlement`**.
