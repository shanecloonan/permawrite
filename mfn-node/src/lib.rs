...
//!   **M2.3.7** [`network::P2P_HANDSHAKE_IO_TIMEOUT`] on outbound TCP dials matches accept-side timeouts. [`NetworkConfig`] holds listener/dial defaults for future full gossip.
...
//!   Integration tests
//!   (`tests/mfnd_smoke.rs`, M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.3 + M2.3.4 + M2.3.5 + M2.3.6 + M2.3.7) drive `serve` over TCP
...
//!   **M2.3.3** `serve --p2p-listen` additionally spawns one `std::thread` for P2P accepts (**M2.3.5**: hello + ping/pong; **M2.3.7**: per-socket read/write timeouts) only; **M2.3.6** `serve --p2p-dial` may spawn one more for outbound dials (**M2.3.7**: same timeout constant as accepts).
