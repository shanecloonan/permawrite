...
- **`light_follow`** — **`GetLightFollowV1`** / **`LightFollowV1`** request/response for light-wallet header + validator-evolution batches, with capped response row counts on encode/decode and bounded interleaved production/gossip frame skipping while waiting for a reply.
