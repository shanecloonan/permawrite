...
| [`bin/mfnd`](src/bin/mfnd.rs) | **M2.1.1** process entrypoint calling [`mfn_node::mfnd_main`]; CLI lives in [`mfnd_cli`](src/mfnd_cli.rs). **M2.1.3** adds `step` (solo produce / apply / save). |
...
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails.
