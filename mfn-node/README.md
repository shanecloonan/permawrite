...
| [`mfnd_serve`](src/mfnd_serve.rs) | **M2.1.6 + M2.1.8 + M2.1.8.1 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13** blocking TCP control plane: `run_serve` accepts one UTF-8 line per connection, returns one **JSON-RPC 2.0** line, closes. Methods `get_tip`, `submit_tx` (hex `encode_transaction` bytes), **`get_block`** (full `encode_block` hex), **`get_block_header`** (`block_header_bytes` + `block_id`), **`get_mempool`** (`mempool_len` + sorted `tx_ids`), **`get_mempool_tx`** (lookup by `tx_id`); **`submit_tx`** `params` may be `{"tx_hex":"…"}` or `["…"]`. |
...
- **Unit (`mfnd_serve::tests`, M2.1.8 + M2.1.8.1 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13)**: 33 tests — JSON-RPC envelope, `submit_tx` object + array `params`, empty array, bad `params[0]`, non-object/array `params`, missing `params`, **`get_block`** / **`get_block_header`** height / params / validated-read error path, header vs full-block agreement, **`get_mempool`** (omit / `{}` / `[]` / reject non-empty `params`), **`get_mempool_tx`** (params / hex / not-found / type guards), and related guards.
...
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails, **`step --blocks 3`** reaches height 3 in one invocation, **`step --blocks 2 --checkpoint-each`** emits two `step_checkpoint` lines, **`mfnd_step_block_log_passes_validated_read`** (`read_block_log_validated` after one `step`), **`serve --rpc-listen 127.0.0.1:0`** + TCP `get_tip` (`mfnd_serve_get_tip_over_tcp`) + **`mfnd_serve_get_tip_jsonrpc_echoes_id`** + **`mfnd_serve_get_mempool_over_tcp_empty`** + **`mfnd_serve_get_mempool_lists_tx_after_submit`** (also **`get_mempool_tx`** round-trip vs fixture `tx_hex`), **`mfnd_serve_get_block_over_tcp_after_step`** (also **`get_block_header`** vs `read_block_log_validated`), **`serve`** `submit_tx` negative paths (`mfnd_serve_submit_tx_rejects_*`, **`mfnd_serve_submit_tx_array_params_rejects_bad_hex`**), **`mfnd_step_writes_block_log_then_serve_submit_tx_admits_transfer`** (synth-decoy genesis + `chain.blocks` + signed transfer admitted as `Fresh`).
