...
|---|---|
| [`emission`](src/emission.rs) | Hybrid emission curve (Bitcoin halvings → Monero tail), fee-split bps, **B-309** fee-shift helper, **B-311** PM41 backstop mint cap, **B-312** PM22 runway metric, **B-313** recommended tail (`>> 9`). |
...
- **Bond wire** (`bond_op_round_trip`, `bond_register_wire_matches_protocol_golden_vector`, `bond_unbond_wire_matches_protocol_golden_vector`, `register_sig_is_bound_to_bls_pk_and_payload`, `register_signing_hash_is_domain_separated`, `unbond_op_round_trip_and_sig_verify`, `unbond_signing_hash_is_domain_separated`, `unbond_sig_does_not_verify_under_different_index`, `unbond_decode_rejects_trailing_bytes`).
...
- **Integration** (`tests/integration.rs`, 26 tests) — multi-block flows: genesis → block1 → block2 with privacy tx, storage upload, slashing; full `unbond_lifecycle` with 3 validators, BLS finality, request → delay → settle, equivocation-during-delay still slashes, exit-churn cap spills across blocks; post-sign header tamper rejects for `validator_root`, `slashing_root`, `storage_proof_root`; legacy and validator mixed CLSAG fee + SPoRA forward-apply plus legacy/validator two-block treasury ledger identity and legacy/validator tampered-CLSAG, tampered-`storage_proof_root`, and duplicate-SPoRA-proof rollback plus validator invalid-coinbase and sub-quorum finality rollback after prior valid history.
- **Block apply** (`tests/block_apply.rs`, 61 tests) — root mismatches; SPoRA binding (emit order, tampered root before payout, dual-proof payout, body tamper atomic reject, positive-yield dual-proof accrual, `proof_reward_window` cap, provenance + treasury on accept, unknown/wrong-chunk/duplicate rejects).
- **Emission simulation (M5.0–M5.47)** (`tests/emission_simulation.rs`, 38 default + 2 nightly `#[ignore]`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains (**96-block** validator CLSAG-only and **64-block** validator mixed CLSAG+SPoRA in CI, **M5.35** / **M5.34/B-03**; **384-block** legacy mixed fee+proof in CI, **M5.39**; **64-block** combined-inflow + PPB + equivocation-PPB in CI, **M5.40**; **128-block** PPB + equivocation combined-inflow in CI, **M5.41**; **256-block** combined-inflow in CI, **M5.42**; **256-block** PPB combined-inflow in CI, **M5.43**; **512-block** combined-inflow + PPB + equivocation combined-inflow in CI, **M5.44**/**M5.45**; **256-block** equivocation combined-inflow + **1M-height** curve in CI, **M5.47**), liveness-slash, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; 2048-block CLSAG fee + 100k empty `apply_block` `#[ignore]` (nightly).
- **Producer treasury settlement (M5 economics, 20 tests)** (`tests/producer_treasury_settlement.rs`) — 90/10 fee split, coinbase = emission + producer share + storage rewards (+ PPB bonus), treasury drain vs backstop, invalid/overpaid coinbase reject without state change, bond/slash/liveness/fee/proof closed loops, five-path and six-path treasury composition, PPB carry-over, and PPB-augmented proof-drain composition.
- **apply_block proptest (M5.2–M5.39)** (`tests/apply_block_proptest.rs`, 37 default, all deep chains in CI) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain) + bond inflow/random fee/SPoRA treasury identity + **M5.33** mixed CLSAG fee + NEW storage upload same-block treasury (`prop_mixed_clsag_fee_and_storage_upload_treasury`) + **M5.38** 64-block deep upload treasury chain (`deep_mixed_clsag_fee_and_storage_upload_treasury_64`; restores **M5.35** CI coverage) + **M5.36** 64-block deep CLSAG+SPoRA treasury chain (`deep_mixed_clsag_fee_and_storage_proof_treasury_64`) + **M5.37** 128-block empty chain + 32-block SPoRA proof chain + 32-block validator mixed CLSAG+SPoRA treasury chain (`deep_empty_block_chain_128`, `deep_storage_proof_chain_32`, `deep_validator_mixed_clsag_fee_and_storage_proof_treasury_32`) + **M5.39** alternating bond-register + SPoRA treasury chain through epoch churn cap (`deep_alternating_register_storage_treasury_8`) + validator combined-inflow, equivocation combined-inflow, and random-schedule treasury identity with equivocation and no-equivocation variants.
- **Validator finality evolution (M5 consensus)** (`tests/validator_finality_evolution.rs`, 69 tests) — pre-block `validator_root` quorum, liveness stats atomicity, bond/slashing/tx/storage_proof/claims/storage/utxo root mismatch reject without state change, invalid/duplicate slash evidence reject without state change, reject preserves state, root movement on liveness/equivocation slash, equivocation during unbond delay, unbond delay preserves root, settlement moves successor root, exit-churn cap deferral/reset, entry-churn cap reject/accept/reset, bond rejection preserving treasury, and bond-op admission rejects for duplicate VRF, duplicate unbond, duplicate pending unbond, below-minimum stake, same-block register-then-unbond, forged/unknown/zombie unbond, and same-block duplicate VRF.
