...
**Emission / treasury (M5.0–M5.16):** default CI runs `mfn-consensus/tests/emission_simulation.rs` (100k-height curve + 10k empty blocks + 512-block storage-proof ledger + validator CLSAG/mixed fee chains + liveness-slash/combined-inflow treasury ledgers, including 32-block equivocation combined-inflow). Longer sims, including 64/256-block combined-inflow and 512-block equivocation combined-inflow runs, are `#[ignore]` (nightly).
