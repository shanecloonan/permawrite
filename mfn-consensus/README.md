...
- **apply_block proptest (M5.2–M5.17)** (`tests/apply_block_proptest.rs`, 28 default + 6 nightly `#[ignore]`) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain) + bond inflow/random fee/SPoRA treasury identity + validator combined-inflow, equivocation combined-inflow, and random-schedule treasury identity with equivocation and no-equivocation variants.
