...
| [`network`](src/network/mod.rs) | **M2.3.0–M2.3.7** P2P scaffold: [`NetworkConfig`](src/network/mod.rs); **M2.3.1** [`frame`](src/network/frame.rs) + **M2.3.5** [`PingV1`](src/network/frame.rs) / [`PongV1`](src/network/frame.rs); **M2.3.2** [`handshake`](src/network/handshake.rs) + **M2.3.4** / **M2.3.5** dial helpers + **M2.3.7** [`P2P_HANDSHAKE_IO_TIMEOUT`](src/network/handshake.rs); **M2.3.3** **`mfnd serve --p2p-listen`** (hello + listener pong, 30s read/write timeouts on accepted sockets); **M2.3.6** **`mfnd serve --p2p-dial`** (outbound full peer handshake). |
...
| [`mfnd_serve`](src/mfnd_serve.rs) | **M2.1.6 + M2.1.8 + M2.1.8.1 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.6 + M2.3.7** blocking TCP control plane: `run_serve` accepts one UTF-8 line per connection, returns one **JSON-RPC 2.0** line, closes. Methods `get_tip`, `submit_tx` (hex `encode_transaction` bytes), **`get_block`** (full `encode_block` hex), **`get_block_header`** (`block_header_bytes` + `block_id`), **`get_mempool`** (`mempool_len` + sorted `tx_ids`), **`get_mempool_tx`** (lookup by `tx_id`), **`remove_mempool_tx`** (`Mempool::evict` by `tx_id`), **`clear_mempool`** (`Mempool::clear`), **`get_checkpoint`** (`Chain::encode_checkpoint` hex), **`save_checkpoint`** (`ChainStore::save`), **`list_methods`** (sorted JSON string names), **`get_claims_for`** / **`get_claims_by_pubkey`** / **`list_recent_uploads`** / **`list_recent_claims`** / **`list_data_roots_with_claims`** (authorship + storage discovery); **`submit_tx`** `params` may be `{"tx_hex":"…"}` or `["…"]`. **M2.3.3** / **M2.3.5** optional **`--p2p-listen`**: second `TcpListener`, stdout `mfnd_p2p_listening=…`, background accept thread: [`hello_v1_handshake`](src/network/handshake.rs) + [`recv_ping_send_pong`](src/network/handshake.rs) per peer (30s socket timeouts; main thread still runs JSON-RPC). **M2.3.6** optional **`--p2p-dial`**: outbound [`tcp_connect_peer_v1_handshake`](src/network/handshake.rs); stdout `mfnd_p2p_dial_ok=…` on success. |
| [`bin/mfnd`](src/bin/mfnd.rs) | **M2.1.1** process entrypoint calling [`mfn_node::mfnd_main`]; CLI in [`mfnd_cli`](src/mfnd_cli.rs). **M2.1.3** `step`; **M2.1.4** mempool drain + fee coinbase + `--blocks N`; **M2.1.5** `--checkpoint-each`; **M2.1.6** `serve` + `--rpc-listen`; **M2.1.7** `chain.blocks` on `step`; **M2.1.8** JSON-RPC responses on `serve`; **M2.1.8.1** array `params` for `submit_tx`; **M2.1.10** `get_block` on `serve`; **M2.1.11** `get_block_header` on `serve`; **M2.1.12** `get_mempool` on `serve`; **M2.1.13** `get_mempool_tx` on `serve`; **M2.1.14** `remove_mempool_tx` on `serve`; **M2.1.15** `clear_mempool` on `serve`; **M2.1.16** `get_checkpoint` on `serve`; **M2.1.17** `save_checkpoint` on `serve`; **M2.1.18** `list_methods` on `serve`; **M2.2.8** authorship discovery RPCs on `serve`; **M2.2.10** derived discovery views on `serve`; **M2.3.3** `serve --p2p-listen`; **M2.3.5** P2P wire (ping/pong after hello); **M2.3.6** `serve --p2p-dial`; **M2.3.7** shared P2P TCP I/O timeouts. |
...
- **Unit (`network::*`, M2.3.0–M2.3.7)**: 20 tests — [`NetworkConfig::default`](src/network/mod.rs); [`network::frame`](src/network/frame.rs) length-prefix + [`HelloV1`](src/network/frame.rs) + **M2.3.5** [`PingV1`](src/network/frame.rs) / [`PongV1`](src/network/frame.rs); [`network::handshake`](src/network/handshake.rs) TCP hello, `tcp_connect_hello_v1_handshake`, `tcp_connect_peer_v1_handshake`, **`tcp_connect_peer_v1_handshake_sets_io_timeouts`**, foreign-genesis rejection, invalid first frame.
- **Unit (`mfnd_serve::tests`, M2.1.8 + M2.1.8.1 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10)**: 70 tests — JSON-RPC envelope, `submit_tx` object + array `params`, empty array, bad `params[0]`, non-object/array `params`, missing `params`, **`get_block`** / **`get_block_header`** height / params / validated-read error path, header vs full-block agreement, **`get_mempool`** / **`clear_mempool`** / **`get_checkpoint`** / **`save_checkpoint`** / **`list_methods`** (omit / `{}` / `[]` / reject non-empty `params`), **`get_mempool_tx`** / **`remove_mempool_tx`** (params / hex / not-found or absent / type guards), **`get_checkpoint`** wire vs `Chain::encode_checkpoint`, **`save_checkpoint`** writes `chain.checkpoint`, **`list_methods`** sorted dispatch set, **`get_claims_for`** / **`get_claims_by_pubkey`** / **`list_recent_uploads`** / **`list_recent_claims`** / **`list_data_roots_with_claims`** params + empty-chain behavior, and related guards.
- **Unit (`mfnd_cli::tests`, M2.1.1–M2.1.8.1 + M2.3.3 + M2.3.6)**: 15 tests — `status` / `step` / `serve` argument shapes, `--genesis`, `--blocks`, `--checkpoint-each`, **`--rpc-listen`** / **`--p2p-listen`** / **`--p2p-dial`** (including rejection unless `serve`), missing `--data-dir`, and step without env seeds.
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.3 + M2.3.4 + M2.3.5 + M2.3.6 + M2.3.7)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails, **`step --blocks 3`** reaches height 3 in one invocation, **`step --blocks 2 --checkpoint-each`** emits two `step_checkpoint` lines, **`mfnd_step_block_log_passes_validated_read`** (`read_block_log_validated` after one `step`), **`serve --rpc-listen 127.0.0.1:0`** + TCP `get_tip` (`mfnd_serve_get_tip_over_tcp`) + **`mfnd_serve_p2p_hello_handshake_over_tcp`** (`--p2p-listen`, `get_tip` genesis hex → [`tcp_connect_peer_v1_handshake`](src/network/handshake.rs)) + **`mfnd_serve_p2p_dial_hits_peer_listener`** (peer B `--p2p-dial` to peer A's `--p2p-listen`, reads `mfnd_p2p_dial_ok=`) + **`mfnd_serve_get_tip_jsonrpc_echoes_id`** + **`mfnd_serve_get_mempool_over_tcp_empty`** + **`mfnd_serve_get_mempool_lists_tx_after_submit`** (non-empty pool, **`get_mempool_tx`** round-trip, **`remove_mempool_tx`**, empty **`get_mempool`**) + **`mfnd_serve_clear_mempool_after_submit`** + **`mfnd_serve_get_checkpoint_round_trips_over_tcp_after_step`** + **`mfnd_serve_save_checkpoint_creates_checkpoint_file`** + **`mfnd_serve_list_methods_over_tcp`** + **`mfnd_serve_authorship_discovery_rpcs_over_tcp`**, **`mfnd_serve_get_block_over_tcp_after_step`** (also **`get_block_header`** vs `read_block_log_validated`), **`serve`** `submit_tx` negative paths (`mfnd_serve_submit_tx_rejects_*`, **`mfnd_serve_submit_tx_array_params_rejects_bad_hex`**), **`mfnd_step_writes_block_log_then_serve_submit_tx_admits_transfer`** (synth-decoy genesis + `chain.blocks` + signed transfer admitted as `Fresh`).
...
- `#![forbid(unsafe_code)]`.
- No background threads in core chain/mempool/store APIs; no clocks, no async runtime. **`mfnd serve`** runs JSON-RPC on a blocking **`std::net::TcpListener`** on the main thread (one client at a time). **M2.3.3** `serve --p2p-listen` adds one background **`std::thread`** for P2P accepts (**M2.3.5**: hello + ping/pong; **M2.3.7**: 30s per-socket I/O timeouts). **M2.3.6** `serve --p2p-dial` adds another for outbound dials (**M2.3.7**: same timeout on the connected stream). Full gossip / libp2p / `tokio` remain future milestones.
...
No async runtime. **`serve`** uses stdlib TCP only (no `tokio`, no libp2p); **M2.3.3** optional **`--p2p-listen`** adds a second stdlib listener on a background thread; **M2.3.6** optional **`--p2p-dial`** runs [`tcp_connect_peer_v1_handshake`](src/network/handshake.rs) on another (**M2.3.7**: 30s read/write timeouts on those dial streams, shared with accept-side P2P sockets). The M2.1.0 on-disk store is a narrow stdlib filesystem adapter over checkpoint bytes; richer RocksDB/sled storage lands later with full architectural intent.
