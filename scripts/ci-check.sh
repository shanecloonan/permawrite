...
soft_ckpt_plan="$(bash scripts/public-devnet-v1/light-scan-checkpoint-soft-rehearsal-smoke.sh)"
[[ "$soft_ckpt_plan" == *"light-scan-checkpoint-soft-rehearsal-smoke: PASS plan-only"* ]] || { printf '%s\n' "$soft_ckpt_plan" >&2; exit 1; }
priv_floor_plan="$(bash scripts/public-devnet-v1/wallet-privacy-floor-rehearsal-smoke.sh)"
[[ "$priv_floor_plan" == *"wallet-privacy-floor-rehearsal-smoke: PASS plan-only"* ]] || { printf '%s\n' "$priv_floor_plan" >&2; exit 1; }
...
frontend_plan="$(bash scripts/public-devnet-v1/vps-start-testnet-frontend-rehearsal-smoke.sh --plan-only)"
[[ "$frontend_plan" == *"vps-start-testnet-frontend-rehearsal-smoke: PASS plan-only"* ]] || { printf '%s\n' "$frontend_plan" >&2; exit 1; }
...
python3 - "$signoff_validate_dir/funded-genesis.json" "$signoff_validate_dir/funded-economy.json" "$signoff_validate_dir/clone-payout-go.json" <<'PY'
import json
import sys
...
with open(sys.argv[3], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
if bash scripts/public-devnet-v1/release-signoff-manifest-validate.sh --manifest "$signoff_validate_dir/clone-payout-go.json" >/dev/null 2>&1; then
  echo "release-signoff-manifest-validate.sh accepted a go manifest with cloned bonded operator payout seeds" >&2
  exit 1
fi
python3 - "$signoff_validate_dir/funded-genesis.json" "$signoff_validate_dir/funded-economy.json" "$signoff_validate_dir/clone-vrf-go.json" <<'PY'
import json
import sys
...
with open(sys.argv[3], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
PY
if bash scripts/public-devnet-v1/release-signoff-manifest-validate.sh --manifest "$signoff_validate_dir/clone-vrf-go.json" >/dev/null 2>&1; then
  echo "release-signoff-manifest-validate.sh accepted a go manifest with cloned validator VRF seeds" >&2
  exit 1
fi
