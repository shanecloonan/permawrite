...
**Long-run ledger identity** vs `apply_block` over CLSAG fee, mixed CLSAG + SPoRA, validator-mode chains, liveness-slash blocks, combined bond/slash/fee/proof inflow stacks, and validator combined-inflow random-fee proptests lives in `tests/emission_simulation.rs` and `tests/apply_block_proptest.rs` (M5.0–M5.8; see [`CI.md`](./CI.md)).
