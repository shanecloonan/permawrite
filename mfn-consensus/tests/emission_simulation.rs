...
//! **M5.1+**, **M5.3**, **M5.9**, **M5.11**, **M5.12**, **M5.13**, **M5.16**, **M5.17**, **M5.19**, **M5.22**, **M5.23**, **M5.24**, **M5.25**, **M5.26**, **M5.28**, **M5.29**, **M5.30**, **M5.31**, **M5.32**, **M5.33**, **M5.34**, **M5.35**, **M5.39**, **M5.40**, **M5.41**, **M5.42**, **M5.43**, **M5.44**, **M5.45**, **M5.46**, **M5.47**, **M5.48**).
//!
//! Fast curve checks (including 1M-height summation, **M5.47**) run in default CI; only the
//! 2048-block CLSAG fee mix (~13m release) and 100k empty `apply_block` (~7m release) harnesses
//! stay `#[ignore]` (**M5.48** tier closure; see `scripts/ci-ignored.sh` / nightly).
...
const SIM_INPUT_PAD_VALUE: u64 = 1_000_000;
...
fn run_mixed_fee_and_proof_sim(blocks: u32, emission: EmissionParams) {
    ...
    let initial = 50_000_000_000u64;
    let (mut st, mut spend, mut input_pad) =
        genesis_with_funded_utxo_and_storage(emission, initial, &storage);
    let mut model_treasury = 0u128;
...
fn run_fee_treasury_sim(blocks: u32, emission: EmissionParams) {
    let initial = 50_000_000_000u64;
    let (mut st, mut spend, mut input_pad) = genesis_with_funded_utxo(emission, initial);
    let mut model_treasury = 0u128;
    for h in 1..=blocks {
...
fn debug_genesis_ring_size_is_sixteen() {
    let (_, spend, _) = genesis_with_funded_utxo(SIM_EMISSION, 1_000);
...
/// Full million-height curve check (~0.1s release). **M5.47** — default CI.
#[test]
...
/// 2048-block CLSAG fee treasury ledger (**M5.48** nightly-only; ~13m release; not promoted to CI).
#[test]
#[ignore = "long CLSAG fee treasury simulation (~13m release); M5.48 nightly-only tier closure; run with cargo test -p mfn-consensus -- --ignored"]
...
fn run_liveness_slash_mixed_treasury_sim(emission: EmissionParams) {
    ...
    let initial = 50_000_000_000u64;
    let (mut st, mut spend_state, mut input_pad) = genesis_validator_with_funded_utxo(
        emission,
        initial,
        &fixture,
        vec![storage.built.commit.clone()],
    ...
    for h in 1..=4u32 {
        let fee = 2_500u64 + u64::from(h % 4_501);
        let (signed, next_spend, next_pad) = spend_state.sign_self_transfer(&input_pad, fee);
...
/// 64-block bond/liveness/fee/proof combined-inflow treasury ledger (**M5.40**).
#[test]
...
/// 512-block bond/liveness/fee/proof combined-inflow treasury ledger (**M5.44**).
#[test]
...
/// 64-block bond/liveness/fee/PPB-proof combined-inflow treasury ledger (**M5.40**).
#[test]
fn treasury_ledger_matches_sixty_four_combined_inflow_ppb_blocks() {
    run_combined_inflow_ppb_treasury_sim(64, SIM_EMISSION, 0);
}

/// 128-block bond/liveness/fee/PPB-proof combined-inflow treasury ledger (**M5.41**).
#[test]
...
/// 256-block bond/liveness/fee/PPB-proof combined-inflow treasury ledger (**M5.43**).
#[test]
...
/// 512-block bond/liveness/fee/PPB-proof combined-inflow treasury ledger (**M5.45**).
#[test]
...
/// 64-block equivocation bond/liveness/fee/PPB-proof combined-inflow ledger (**M5.40**).
#[test]
...
/// 128-block equivocation bond/liveness/fee/PPB-proof combined-inflow ledger (**M5.41**).
#[test]
...
/// 128-block equivocation bond/liveness/fee/proof combined-inflow ledger (**M5.41**).
#[test]
...
/// 256-block equivocation bond/liveness/fee/proof combined-inflow ledger (**M5.47**).
#[test]
fn treasury_ledger_matches_two_hundred_fifty_six_equivocation_combined_inflow_blocks() {
    run_equivocation_combined_inflow_treasury_sim(256, SIM_EMISSION, 0);
}

/// 256-block equivocation bond/liveness/fee/PPB-proof combined-inflow ledger (**M5.47**).
#[test]
fn treasury_ledger_matches_two_hundred_fifty_six_equivocation_combined_inflow_ppb_blocks() {
    run_equivocation_combined_inflow_ppb_treasury_sim(256, SIM_EMISSION, 0);
}

/// 512-block equivocation bond/liveness/fee/proof combined-inflow ledger (**M5.45**).
#[test]
...
/// 512-block equivocation bond/liveness/fee/PPB-proof combined-inflow ledger (**M5.45**).
#[test]
...
/// 100k empty legacy `apply_block` chain (**M5.48** nightly-only; ~7m release; not promoted to CI).
#[test]
#[ignore = "long apply_block simulation (~7m release); M5.48 nightly-only tier closure; run with cargo test -p mfn-consensus -- --ignored"]
