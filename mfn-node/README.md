...
- **Unit (`mfnd_serve::tests`, M2.1.8 + M2.1.8.1 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16)**: 49 tests — JSON-RPC envelope, `submit_tx` object + array `params`, empty array, bad `params[0]`, non-object/array `params`, missing `params`, **`get_block`** / **`get_block_header`** height / params / validated-read error path, header vs full-block agreement, **`get_mempool`** / **`clear_mempool`** / **`get_checkpoint`** (omit / `{}` / `[]` / reject non-empty `params`), **`get_mempool_tx`** / **`remove_mempool_tx`** (params / hex / not-found or absent / type guards), **`get_checkpoint`** wire vs `Chain::encode_checkpoint`, and related guards.
...
- **Integration (`tests/mfnd_smoke.rs`, M2.1.1 + M2.1.2 + M2.1.3 + M2.1.4 + M2.1.5 + M2.1.6 + M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16)**: spawns the real `mfnd` binary — `status` / `save` / `--genesis` boot, missing `--data-dir` error path, **`step`** twice on `devnet_one_validator.json` with env seeds (tip reaches height 2 + checkpoint on disk), **`step`** without env fails, **`step`** on empty-validator demo genesis fails, **`step --blocks 3`** reaches height 3 in one invocation, **`step --blocks 2 --checkpoint-each`** emits two `step_checkpoint` lines, **`mfnd_step_block_log_passes_validated_read`** (`read_block_log_validated` after one `step`), **`serve --rpc-listen 127.0.0.1:0`** + TCP `get_tip` (`mfnd_serve_get_tip_over_tcp`) + **`mfnd_serve_get_tip_jsonrpc_echoes_id`** + **`mfnd_serve_get_mempool_over_tcp_empty`** + **`mfnd_serve_get_mempool_lists_tx_after_submit`** (non-empty pool, **`get_mempool_tx`** round-trip, **`remove_mempool_tx`**, empty **`get_mempool`**) + **`mfnd_serve_clear_mempool_after_submit`** + **`mfnd_serve_get_checkpoint_round_trips_over_tcp_after_step`**, **`mfnd_serve_get_block_over_tcp_after_step`** (also **`get_block_header`** vs `read_block_log_validated`), **`serve`** `submit_tx` negative paths (`mfnd_serve_submit_tx_rejects_*`, **`mfnd_serve_submit_tx_array_params_rejects_bad_hex`**), **`mfnd_step_writes_block_log_then_serve_submit_tx_admits_transfer`** (synth-decoy genesis + `chain.blocks` + signed transfer admitted as `Fresh`).
