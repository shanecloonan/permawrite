...
- **Emission simulation (M5.0–M5.19)** (`tests/emission_simulation.rs`, 23 tests) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains, liveness-slash, combined-inflow, 32/64-block equivocation combined-inflow ledgers, and prefunded treasury backstop checks; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
