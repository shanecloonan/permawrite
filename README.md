...
[![Tests](https://img.shields.io/badge/tests-699_passing-brightgreen)](#status)
...
**Pre-testnet.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` binary exercises checkpoint persistence, optional JSON genesis (`--genesis`), solo **`step`** (mempool drain + coinbase + produce / apply / save, with **`--blocks N`** and optional **`--checkpoint-each`** for per-block checkpoints), an append-only **`chain.blocks`** log after each applied block (**M2.1.7**) with optional **validated replay vs checkpoint (M2.1.9)**, env-gated validator seeds, and **`serve`** (blocking TCP JSON-RPC plus optional **`--p2p-listen`** / **`--p2p-dial`** with **M2.3.16** post-handshake **tx/block gossip** over length-prefixed frames). There is still no multi-peer block sync when the remote tip is ahead, HTTP/WebSocket RPC, or public testnet. As of the latest commit:
...
| P2P wire + gossip           | [`mfn-net`](./mfn-net/README.md) |  —  | Framing, handshake, **M2.3.16** **`TxV1`** / **`BlockV1`** gossip. |
| Node daemon (`mfnd`)        | [`mfn-node`](./mfn-node/README.md) |  176  | **M2.0.3 `Chain` driver** (re-exported from **`mfn-runtime`**) + **M2.0.4 producer helpers** + **M2.0.5 light-header agreement tests** + **M2.0.12 `Mempool`** + **M2.0.13 storage-anchoring admission** + **M2.0.15 chain-state persistence adapter** + **M2.1.0 `ChainStore` filesystem checkpoint store** + **M2.1.1 `mfnd` binary (`status` / `save` / `run`)** + **M2.1.2 JSON `--genesis` chain spec** + **M2.1.3 `mfnd step`** + **M2.1.4 mempool-aware `step` + `--blocks N`** + **M2.1.5 `--checkpoint-each`** + **M2.1.6 `mfnd serve` (TCP one-line JSON `get_tip` / `submit_tx`, `--rpc-listen`)** + **M2.1.6.1 subprocess TCP tests for `submit_tx` error paths** + **M2.1.7 `chain.blocks` append log + optional genesis `synthetic_decoy_utxos` + signed `submit_tx` over `serve`** + **M2.1.8 JSON-RPC 2.0 response envelope on `serve`** + **M2.1.8.1 `submit_tx` array `params`** + **M2.1.9 `ChainStore::read_block_log_validated`** + **M2.1.10 `serve` `get_block`** + **M2.1.11 `serve` `get_block_header`** + **M2.1.12 `serve` `get_mempool`** + **M2.1.13 `serve` `get_mempool_tx`** + **M2.1.14 `serve` `remove_mempool_tx`** + **M2.1.15 `serve` `clear_mempool`** + **M2.1.16 `serve` `get_checkpoint`** + **M2.1.17 `serve` `save_checkpoint`** + **M2.1.18 `serve` `list_methods`** + **M2.2.8 `serve` authorship discovery (`get_claims_for`, `get_claims_by_pubkey`, `list_recent_uploads`)** + **M2.2.10 derived discovery views (`list_recent_claims`, `list_data_roots_with_claims`)** + **M2.3.0 `network` scaffold (`NetworkConfig`)** (solo-validator produce → `apply` → `remove_mined` → `save`; coinbase includes producer share of drained tx fees per `fee_to_treasury_bps`; optional per-block checkpoint during multi-`step` runs) — chain driver owning `ChainState`, applying blocks sequentially through `apply_block`, a three-stage block-production protocol (`build_proposal` → `vote_on_proposal` → `seal_proposal`) with a `produce_solo_block` one-call convenience, the in-memory transaction pool that bridges `mfn-wallet` to the producer, restart-safe `Chain::checkpoint` / `Chain::encode_checkpoint` / `Chain::from_checkpoint_bytes`, and now a narrow IO boundary that saves/restores canonical checkpoint bytes via `chain.checkpoint`, `chain.checkpoint.bak`, and `chain.checkpoint.tmp`. `Mempool::admit` replicates every per-tx gate `apply_block` runs, including privacy and permanence gates; end-to-end integration tests drive wallet → mempool → producer → chain → light-client flows for both transfers and storage uploads. |
...
| Wallet CLI (`mfn-cli`)     | [`mfn-cli`](./mfn-cli/README.md) |  21  | **M3.0–M3.15** TCP JSON-RPC client + `wallet` subcommands: full/light scan, send/upload/claim, claims/uploads index queries, weak-subjectivity export/import/pin for light wallets. |
