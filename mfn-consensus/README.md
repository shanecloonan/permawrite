...
- **Emission simulation (M5.0–M5.7)** (`tests/emission_simulation.rs`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains, liveness-slash and combined-inflow ledgers, coinbase decrypt checks; deep sims `#[ignore]` (nightly).
...
- **apply_block proptest (M5.2–M5.8)** (`tests/apply_block_proptest.rs`, 29 tests) — randomized valid chains + reject-without-mutation props; duplicate-proof rollback + **legacy mixed reject matrix** (tampered root, invalid CLSAG, reject-after-partial-chain) + **validator-mixed reject matrix** (tampered root, bad coinbase, subquorum, invalid CLSAG, reject-after-partial-chain) + bond inflow/random fee/SPoRA treasury identity + validator combined-inflow random-fee treasury identity.
