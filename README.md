...
[![Tests](https://img.shields.io/badge/tests-349_passing-brightgreen)](#status)
...
| Node-side glue              | [`mfn-node`](./mfn-node/README.md) |   14  | **M2.0.3 `Chain` driver** + **M2.0.4 producer helpers** — in-memory chain driver owning `ChainState`, applying blocks sequentially through `apply_block`, plus a three-stage block-production protocol (`build_proposal` → `vote_on_proposal` → `seal_proposal`) with a `produce_solo_block` one-call convenience. Demonstrated end-to-end via a 1-validator BLS-signed 3-block integration test. The orchestration foundation every M2.x sub-milestone (mempool, RPC, store, P2P) will attach around. |
...
| **Total** | | **349** | Zero `unsafe`. Zero clippy warnings. |
...
# Run the full workspace test suite (349 tests, ~30s on a modern machine)
