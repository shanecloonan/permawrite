...
- **Emission simulation (M5.0–M5.30)** (`tests/emission_simulation.rs`, 18 default + 19 nightly `#[ignore]`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains, liveness-slash, combined-inflow, 32/64-block equivocation combined-inflow ledgers, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
