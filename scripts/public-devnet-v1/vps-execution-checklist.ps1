...
if ($Json) {
    ...
    Write-Host "  4. $($report.commands.tl5_assert)"
    Write-Host "  5. $($report.commands.tl6_rehearsal)  # archive vps-participant-rehearsal-*.txt"
    Write-Host "  6. $($report.commands.tl6_assert)"
    Write-Host "  7. $($report.commands.archive)"
    Write-Host "  8. $($report.commands.ceremony)"
    Write-Host "  9. $($report.commands.tl7_signoff)  # human sign-off"
    Write-Host " 10. $($report.commands.tl8_publish_seeds)  # commit manifest"
    Write-Host " 11. $($report.commands.tl8_publish_checkpoint_log)  # commit checkpoints.jsonl"
    Write-Host " 12. $($report.commands.tl8_invite)  # share invite packet"
    Write-Host " 13. $($report.commands.tl9_launch_gate)"
}
