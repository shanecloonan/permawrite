...
**Tests:** 205 passing (161 unit + 44 integration) &nbsp;·&nbsp; **`unsafe`:** forbidden &nbsp;·&nbsp; **Clippy:** clean
...
| [`network`](src/network/mod.rs) | **M2.3.0–M2.3.14** P2P scaffold: [`NetworkConfig`](src/network/mod.rs); **M2.3.1** [`frame`](src/network/frame.rs) + **M2.3.5** [`PingV1`](src/network/frame.rs) / [`PongV1`](src/network/frame.rs) + **M2.3.8** [`ChainTipV1`](src/network/frame.rs) + **M2.3.10** [`GoodbyeV1`](src/network/frame.rs); **M2.3.2** [`handshake`](src/network/handshake.rs) + **M2.3.4** / **M2.3.5** dial helpers + **M2.3.7** [`P2P_HANDSHAKE_IO_TIMEOUT`](src/network/handshake.rs) + **M2.3.8** [`tcp_connect_peer_v1_handshake_with_tip_exchange`](src/network/handshake.rs) (includes **M2.3.10** goodbye); **M2.3.3** **`mfnd serve --p2p-listen`** (hello + listener pong + tip + goodbye + **M2.3.9** `mfnd_p2p_peer_tip` + **M2.3.11** `mfnd_p2p_height_cmp` + **M2.3.12** `mfnd_p2p_handshake_ms` + **M2.3.13** `hid=`, 30s read/write timeouts on accepted sockets; **M2.3.14** sequential-accept `hid` smoke); **M2.3.6** **`mfnd serve --p2p-dial`** (outbound full peer handshake + **M2.3.9** + **M2.3.11** + **M2.3.12** + **M2.3.13**). |
