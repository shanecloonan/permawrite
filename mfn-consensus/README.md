...
**Tests:** 383 passing (151 unit + 232 integration across six `tests/` targets; 11 nightly `#[ignore]`) &nbsp;·&nbsp; **`unsafe`:** forbidden &nbsp;·&nbsp; **Clippy:** `-D warnings` (see `scripts/ci-check`)
...
- **Emission simulation (M5.0–M5.42)** (`tests/emission_simulation.rs`, 28 default + 9 nightly `#[ignore]`) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains (**96-block** validator CLSAG-only and **64-block** validator mixed CLSAG+SPoRA in CI, **M5.35** / **M5.34/B-03**; **384-block** legacy mixed fee+proof in CI, **M5.39**; **64-block** combined-inflow + PPB + equivocation-PPB in CI, **M5.40**; **128-block** PPB + equivocation combined-inflow in CI, **M5.41**; **256-block** combined-inflow in CI, **M5.42**), liveness-slash, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
