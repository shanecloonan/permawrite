...
- **Validator finality evolution tests.** **M5 consensus (partial ✓):** `tests/validator_finality_evolution.rs` — pre-block `validator_root` quorum, liveness evolution atomicity, reject preserves state, bond/slashing/**tx**/**storage_proof** root mismatch rejects, invalid slash evidence reject without state change, validator-root movement on slash, equivocation during unbond delay, unbond delay + settlement root semantics, and exit-churn cap deferral (`a97242a`, `267658d`, `4715544`, `b50662f`). **M5.5/M5.6 integration (partial ✓):** `tests/integration.rs` — legacy mixed CLSAG fee + SPoRA forward-apply, validator mixed CLSAG fee + SPoRA forward-apply, and two-block treasury ledger identity (`ba04799`, `30b35c0`, `2063202`).
