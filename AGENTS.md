...
**CI gate (2026-08-22):** **B-314** PM19 cold-proof helper (this commit). Tip CI `#32578440244` on B-313 `9028debb` rust **GREEN** / scripts **FAIL**. Nightly `#31861932921` **GREEN**. Lane7 Path A **22565** / last_proven **22560**. Slash-clone matrix frozen. Strategic path: L4 -> **B-40** -> **B-13a** -> **B-25**. No B-13c; no DEFAULT flip.
...
| **6** Permanence | **B-314** PM19 cold-proof helper (this commit); **B-313** (`9028debb`); **B-312** (`4720c245`); **B-311** (`6c0d4a84`); **B-310** (`98e3adde`) | *Idle* | **B-306b** drip+backstop after B-25 / Path B; **PM1** enable bond floor (also unblocks B-314 wire); **B-20** arm fee-shift after B-13c + B-25; **B-44** wire lottery after B-32 (lane 4); PM8 store wire after archival role; PM41 cap wire after B-306b; PM22 RPC after B-312; **B-313** tail shrink after B-25 / Path B; human **B-33**; no B-13c enable | Emission sims |
...
| --- | --- | --- | --- |
| 6 | 4 | **B-314 PM19:** when wiring per-commit audit after **PM1**, call `cold_proof_overdue` / `recommended_cold_reward_multiplier_bps`; do not treat B5 global any-stale as a per-file obligation. Path A stays global. | **Open** |
...
| B-313 | Tail-emission size helper (`>> 9`; not a schedule flip) | 6 | **Landed** `9028debb` — `recommended_tail_emission`; Path A stays `>> 8`; enable = Path B / post-B-25 |
| B-314 | PM19 per-commitment cold-proof cadence (not wired) | 6 | **Landed** (this commit) — `cold_proof_overdue` / bounty bps; Path A B5 stays global any-stale; wire after **PM1** |
...
1. **2026-08-22 - lane 6 - B-314 PM19 cold-proof helper** (this commit): `recommended_min_proof_interval_slots` ages 7_200 → 216_000 (~30d at 1y); `cold_proof_overdue` / `missed_proof_windows`; bounty 10_000→20_000 bps. Path A B5 stays global any-stale. `b314_*` + genesis pin PASS; clippy `-D warnings` green. Docs: [`B314_COLD_PROOF.md`](docs/B314_COLD_PROOF.md). Waited for tip CI `#32578440244` on B-313 (rust GREEN / scripts FAIL). Full GitHub CI (no skip). No B-13c; no DEFAULT flip; slash matrix frozen; **not B5 wire**. Next: **B-306b** + **PM1** after B-25 / Path B; **B-20** after B-13c + B-25; **B-44** after B-32. *Observed (not staged):* `apply_block_proptest.rs`; `tx_storm.rs`; rc-audit json.

1. **2026-08-22 - lane 6 - B-313 tail-emission helper** (`9028debb`): `recommended_tail_emission` = `initial_reward >> (halving_count+1)` (`50 MFN >> 9` = last subsidy / 4). Path A stays `>> 8` (2×). Tail-start inflation 644_558 → 322_279 ppb if applied. PM13 still forbids 0. `b313_*` + genesis pin PASS; clippy `-D warnings` green. Docs: [`B313_TAIL_EMISSION.md`](docs/B313_TAIL_EMISSION.md). Waited for tip CI `#32575710784` on B-312 (rust GREEN / scripts FAIL). Full GitHub CI (no skip). No B-13c; no DEFAULT flip; slash matrix frozen; **not a schedule flip**. Next: **B-306b** + **PM1** after B-25 / Path B; **B-20** after B-13c + B-25; **B-44** after B-32. *Observed (not staged):* `apply_block_proptest.rs`; `tx_storm.rs`; rc-audit json.
...
1. **2026-08-22 - lane 6 - B-310 PM8 state-retention helper** (`98e3adde`): `archive_retention` classifies all 24 `ChainState` fields (PermanenceKeep / PrivacyKeep / ConsensusKeep). Spent UTXOs stay decoys; storage never prunes; only `chain.blocks` bodies with `h < finalized checkpoint` are candidates. Path A does not drop the log. `b310_*` PASS; clippy `-D warnings` green. Docs: [`B310_STATE_RETENTION.md`](docs/B310_STATE_RETENTION.md). Waited for tip CI `#32569850902` on B-309 (rust GREEN / scripts FAIL). Full GitHub CI (no skip). No B-13c; no DEFAULT flip; slash matrix frozen; **not PM8 wire**. Next: **B-306b** + **PM1** after B-25 / Path B; **B-20** after B-13c + B-25; **B-44** after B-32. *Observed (not staged):* `apply_block_proptest.rs`; `tx_storm.rs`; rc-audit json.

1. **2026-08-22 - lane 6 - B-309 fee-shift helper** (`873e6b1e`): `recommended_fee_to_treasury_bps` holds 9000 unless treasury ≤ B-28 floor **and** backstop is majority of proof blocks, then +1000 saturating at 10000. Path A `fee_to_treasury_bps` stays **9000**. `b309_*` + Path A genesis pin PASS; clippy `-D warnings` green. Docs: [`B309_FEE_SHIFT.md`](docs/B309_FEE_SHIFT.md). Waited for tip CI `#32568097779` on B-308 (rust GREEN / scripts FAIL). Full GitHub CI (no skip). No B-13c; no DEFAULT flip; slash matrix frozen; **not B-20 arm**. Next: **B-306b** + **PM1** after B-25 / Path B; **B-20** after B-13c + B-25; **B-44** after B-32. *Observed (not staged):* `apply_block_proptest.rs`; `tx_storm.rs`; rc-audit json.
