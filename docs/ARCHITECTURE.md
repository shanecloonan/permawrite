...
├── mfnd_serve.rs   M2.1.6 + M2.1.8 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 blocking TCP serve: one-line JSON request/response;
...
└── bin/mfnd.rs     M2.1.1 — `mfnd_main`: status / save / run (Unix: Ctrl+C save;
                    ...
                    M2.1.6–M2.1.8: `serve` — TCP line JSON-RPC (`get_tip`, `submit_tx`, `get_block`, `get_block_header`, `get_mempool`, `get_mempool_tx`, `remove_mempool_tx`); `--rpc-listen`.
