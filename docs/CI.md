...
**apply_block proptest (M5.2 / M5.2+):** default CI runs `mfn-consensus/tests/apply_block_proptest.rs` (32-case `proptest!` for empty chains, header tamper rejects, storage-proof chains, multi-register bond ops in one block; plain `#[test]` for forged-register and duplicate-proof rejects — avoids `proptest!` `$body:block` brace limits). Deep chains are `#[ignore]`.
