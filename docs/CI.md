...
**Producer treasury settlement (M5 economics):** default CI runs `mfn-consensus/tests/producer_treasury_settlement.rs` — 90/10 fee split, coinbase = emission + producer fee share + storage rewards (+ PPB bonus), treasury drain vs emission backstop, invalid/overpaid coinbase reject without state change, bond burn + fee inflow (`f117ce6`), slash + fee + proof + PPB carry-over (`13616bc`), liveness/bond/combined-inflow treasury loops (`ffe93d5`, `cbecb3b`, `5a8fb83`, `40bfb57`).
