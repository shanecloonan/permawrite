...
| Public observer read-RPC proxy | `http://5.161.201.73:8787/rpc` — systemd `observer-rpc-proxy.service` → observer `127.0.0.1:18734` (public-safe methods only; status pages / lite explorers). Tall-tip `get_light_snapshot` / `get_block_headers` use `PROXY_HEAVY_RPC_TIMEOUT_MS` (default 180s; B-52/F54). **B-229:** proxy caches `get_block_header(s)` rows + tip-warms — mfnd still re-reads full `chain.blocks` on cache miss (~3–4s at tip≈16k); `/health` exposes `header_cache_*`. **B-251:** background index `get_tip` uses `PROXY_INDEX_TIP_TIMEOUT_MS` (default 90s) so concurrent tall-tip snapshots do not stall permanence tx-count indexing; `/health` exposes `index_tip_timeout_ms`. Deploy: `bash scripts/public-devnet-v1/vps-update-observer-rpc-proxy.sh --apply` |
...
| Faucet UTXO hygiene (VPS) | **B-278 / F122 / B-296:** Path A faucet shared validator0 `payout_stealth` → ~1 coinbase UTXO/block; at `owned≈20k` sends hang. (1) Stop `faucet-http` when `busy=false`; (2) `bash scripts/public-devnet-v1/faucet-wallet-prune.sh --keep 32`; (3) `faucet-rotate-from-payout.sh` then systemd drop-in `Environment=FAUCET_WALLET=/root/testnet-wallets/faucet-ops.json` + start when idle (need **two** payout funds for F7); (4) prove `POST /faucet` job `done` under 120s. Self-send consolidate cannot shrink under the 2-output privacy floor. Live: B-278 prune 22062→32, job **99s**; **B-296** rotated hot path to `faucet-ops.json`, HTTP job **76s**, storm landed 13, observer user-tx 391→408. **B-15 wave115:** storm left `faucet-ops` **owned=0** (HTTP F7 1-input error); payout refill owned=3 (no faucet restart); iris job **91s**; last_proven **22467**. Evidence `b15-wave115-last-proven-22467-20260815.md`. **B-42:** iris observer upload `24e62a5a` last_proven **22487**; staggered `b42-join-a` `ea7cba7c` last_proven **22492**; **3rd** `b42-join-b` `6a1468fc` last_proven **22560** (payout two 2e6 via hub; B-50 pin; no faucet/mfnd restart). Evidence `b42-join-b-last-proven-22560-20260815.md`. **2026-08-15 faucet HTTP F7:** B-50 pin + payout refill (need **owned≥4** so first dual-send leftover stays ≥2 if change miss); job **`f77a4048` done 73s** tip **22522**. Evidence `b-faucet-http-f7-20260815.md`. **2026-08-15 faucet RPC:** hub produce EAGAIN starves `mfn-cli tip` on `:18731`; live drop-in `MFND_RPC=127.0.0.1:18733` (voter). Hub OOM-kill 07:30Z (systemd restart; do not agent-restart mfnd). Path A land tip-**22495**. Evidence `b42-hub-oom-path-a-22495-20260815.md`. |
| Tall-tip mfnd restart (VPS) | **B-291:** at tip~22k, block-log replay before RPC bind is **~10-15 min**. Set Environment=MFN_SKIP_MANIFEST_SEEDS=1 on hub/voters/observer (avoid public 1900x hairpins). Keep peers.json as {version,max_outbound_peers,peers:[...]} with **loopback committee only** (127.0.0.1:19101-19104) - bare string lists abort load. Restart order: voters first, wait :19102/:19103 listen, then **hub only**, wait RPC, tip +2. Do not thrash wallet light-scan during recover; do not expect fast mfnd save without an existing chain.checkpoint. **B-299:** recover may leave `mfn-p2p-forward-hub` stopped (19001 CLOSED while 19101 still listens). `systemctl start mfn-p2p-forward-hub` only — never mfnd — then prove outside TCP 19001 OPEN and tip +1. Evidence `b299-hub-seed-19001-20260815.md`. **2026-08-15 hub OOM:** snapshot/EAGAIN load can OOM-kill hub (status=9); systemd restarts; replay ~10 min @ tip~22k. After bind, 97% CPU + public-19001 handshake aborts starve vote fanout (proposal sits ~4-9 min). Unstick: `systemctl stop mfn-p2p-forward-hub` only (do not restart mfnd), wait for `producer_sealed`, then `start` again. Submit txs via observer `:18734` when hub RPC is EAGAIN. Evidence `b42-hub-oom-path-a-22486-20260815.md`. **2026-08-15 CLOSE-WAIT:** hub `:19101` refills **48 CLOSE-WAIT** on loopback after hub-only restart even with 19001 down; `inbound_cap_reached cap=48` + no `producer_sealed`; tip stuck **22495**. Restart clears slots for minutes only. Restore `mfn-p2p-forward-hub` after diagnosis (19001 OPEN). Lane 4 owns inbound-slot close. Evidence `b42-tip-22495-close-wait-19001-20260815.md`. **B-300 VPS apply (2026-08-15):** roll `vps-prebuild-mfnd --apply` then `vps-roll-mfnd --apply --skip-pull --skip-build` (`MFN_ROLL_ALLOW_RED_CI=1` while scripts FAIL / rust GREEN). RPC wait **900s**. Tip **22495→22502**; CLOSE-WAIT 47→1; no `inbound_cap_reached`. Observer may systemd-restart and replay ~10-15 min (proxy 502 until `:18734` binds) — do not agent-restart it. Evidence `b300-vps-apply-tip-22502-20260815.md`. |
| VPS mfnd prebuild (no restart) | `bash scripts/public-devnet-v1/vps-prebuild-mfnd.sh --apply` — **B-62/B-65**; builds release bins only; sources `lib-cargo-env.sh` for non-interactive PATH |
| VPS roll readiness assert | `bash scripts/public-devnet-v1/assert-vps-roll-ready.sh` — **B-62**; tip + faucet idle + CI GREEN + B-51 marker |
| VPS mfnd binary roll (hub+voters) | `bash scripts/public-devnet-v1/vps-roll-mfnd.sh --apply` — **B-49/B-60/B-61**; rebuild `mfnd`, soften dial/`Wants=`, restart voters then hub; **never** `faucet-http`; fail-closed: CI GREEN (gh or public API) + faucet idle + RPC listen wait after restart |
...
| **B-42 / B-248 / B-257 invite-load** | `bash scripts/public-devnet-v1/invite-load-smoke-rehearsal.sh --apply` (or `.ps1 -Apply`) — B-15-safe preflight (proxy+faucet+seeds+p2p-forward hygiene); `serialize-with-reason` until clear. Live JOIN arm: `MFN_INVITE_LOAD_ALLOW_LIVE=1` + `--live` (JOIN still operator-driven; never during B-15) |
...
**B-254:** `assert-public-testnet-health.sh --apply` fails closed if any `mfn-p2p-forward@*` is in failed units, and requires the four dedicated forward units active (JOIN-facing P2P honesty).
...
**B-88 / B-269 VPS timer:** `bash scripts/public-devnet-v1/vps-install-near-tip-ckpt-timer.sh --apply` installs `path-a-near-tip-ckpt.timer` (**every 8m**, matched to `MFN_CKPT_LAG_THRESHOLD=8`). After a timer publish dirties the VPS working tree, agents must land the jsonl on `main` (`land-path-a-checkpoint-from-vps` + commit); before `git pull` on the VPS, `git checkout -- mfn-node/testdata/public_devnet_v1.checkpoints.jsonl` if needed.
...
Path A omits it today (`0`). Live same-chain enable needs activation-height
(**B-268** design: [`B13_ACTIVATION_HEIGHT.md`](../../docs/B13_ACTIVATION_HEIGHT.md))
...
Assert evidence: [28-assert-pass-20260806T112316Z.md](./evidence/b28-assert-pass-20260806T112316Z.md). Assert draft floors: ssert-b28-treasury-thresholds.ps1 -Rpc http://5.161.201.73:8787/rpc (default -Mode pre; after B-13c -Mode post / --mode post expects subsidy_bps==1000). B-40 D0 one-shot: 40-d0-preflight.ps1 -Rpc http://5.161.201.73:8787/rpc (or .sh).
...
### B-20 — Fee-shift policy (**helper landed B-309** — arm after B-13c / B-25)

See [`docs/B309_FEE_SHIFT.md`](../../docs/B309_FEE_SHIFT.md) and [`docs/FEES.md`](../../docs/FEES.md) §5.5. Do **not** change
`fee_to_treasury_bps` in the same fork as **B-13c**. Path A stays **9000**. Arm only after permanence
week evidence.
...
| Standalone `wallet claim` errors / claim queries empty | Standalone claim is **disabled**; unbound claims are not indexed | Attach authorship at upload with `wallet upload FILE --message "..."`; after mining, run `claims for <DATA_ROOT_HEX> --json` (and `claims by-pubkey` if needed) against a synced node with the same `genesis_id`. |
