...
- **Block apply** (`tests/block_apply.rs`, 59 tests) — root mismatches; SPoRA binding (emit order, tampered root before payout, dual-proof payout, body tamper atomic reject, positive-yield dual-proof accrual, `proof_reward_window` cap, provenance + treasury on accept, unknown/wrong-chunk/duplicate rejects).
- **Emission simulation (M5.0–M5.30)** (`tests/emission_simulation.rs`, 33 tests) — long-run treasury ledger vs `apply_block`, validator CLSAG-fee and mixed fee+proof chains, liveness-slash, combined-inflow, 32/64-block equivocation combined-inflow ledgers, prefunded treasury backstop checks, and no-equivocation PPB combined-inflow ledger sims; coinbase decrypt checks; deep sims `#[ignore]` (nightly).
