...
| Faucet UTXO hygiene (VPS) | **B-278 / F122:** Path A faucet shares validator0 `payout_stealth` seed → ~1 coinbase UTXO/block; at `owned≈20k` sends hang. (1) Stop `faucet-http` when `busy=false`; (2) `bash scripts/public-devnet-v1/faucet-wallet-prune.sh --keep 32` (keeps tip `scan_height`, drops cache rows); (3) optional `faucet-rotate-from-payout.sh` → `Environment=FAUCET_WALLET=/root/testnet-wallets/faucet-ops.json` + restart faucet when idle; (4) prove `POST /faucet` job `done` under 120s. Self-send consolidate cannot shrink under the 2-output privacy floor — prune/rotate first. Live prove 2026-08-14: prune 22062→32, HTTP dual-fund job **done in 99s**. Hot path stays pruned `validator0-faucet.json` until **B-296** proves ops rotate + calm light-scan. |
| Tall-tip mfnd restart (VPS) | **B-291:** at tip~22k, block-log replay before RPC bind is **~10-15 min**. Set Environment=MFN_SKIP_MANIFEST_SEEDS=1 on hub/voters/observer (avoid public 1900x hairpins). Keep peers.json as {version,max_outbound_peers,peers:[...]} with **loopback committee only** (127.0.0.1:19101-19104) - bare string lists abort load. Restart order: voters first, wait :19102/:19103 listen, then **hub only**, wait RPC, tip +2. Do not thrash wallet light-scan during recover; do not expect fast mfnd save without an existing chain.checkpoint. Evidence 291-tall-tip-mesh-recover-20260815.md. |
