...
- [x] **B8.0** (lanes 4+5) — `mfn-net::transport` + `MFND_P2P_TRANSPORT` / `MFND_TOR_SOCKS5` env knobs (`419e38a`).
- [x] **B8.1** (lanes 4+5) — SOCKS5 CONNECT outbound P2P dial for `MFND_P2P_TRANSPORT=tor` (this push).
...
- [x] **Lane 7 / TL-5 execution checklist** — `vps-execution-checklist.*` read-only gate before VPS soak (this push).
