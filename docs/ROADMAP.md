...
| **M2.4.62** | Mesh stability: seal pending proposals on producer slot tick when quorum arrives between ticks; exclude ephemeral inbound peers from committee catch-up/reconnect/`peers.json`; two-phase soak warmup; observer periodic catch-up + inbound ahead-tip pull on session close. | in progress |
