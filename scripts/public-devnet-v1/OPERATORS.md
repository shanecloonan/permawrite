...
| Faucet UTXO consolidate (VPS) | `bash scripts/public-devnet-v1/faucet-consolidate.sh --plan-only` then real run weekly when `owned_count` grows. **B-277 / F122 (2026-08-07):** live faucet wallet hit `owned_count_cached≈16388` / ~10 MiB / `pending_spent_count≈480` — HTTP faucet jobs and even a 600s direct `wallet send` timed out while tip still advanced. Run consolidate (or rotate faucet wallet) before invite-load / JOIN fund waves; raising `send_timeout_ms` alone is not enough. |
