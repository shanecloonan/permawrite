...
//!   **M2.3.8** [`network::ChainTipV1`] exchange after ping/pong on P2P streams; **M2.3.9** **`mfnd_p2p_peer_tip`** line on stdout after each successful exchange; **M2.3.10** [`network::GoodbyeV1`] after the tip on the full [`network::tcp_connect_peer_v1_handshake_with_tip_exchange`] path; **M2.3.11** **`mfnd_p2p_height_cmp`** (local vs remote height) on stdout after **`mfnd_p2p_peer_tip`**; **M2.3.12** **`mfnd_p2p_handshake_ms`** (wall-clock through that same success point); **M2.3.13** matching **`hid=`** on those three stdout lines; **M2.3.14** integration smoke for sequential accepts advancing **`hid`**. [`NetworkConfig`] holds listener/dial defaults for future full gossip.
...
//!   `127.0.0.1:18731`). **M2.3.3** / **M2.3.5** / **M2.3.6** / **M2.3.8** / **M2.3.9** / **M2.3.10** / **M2.3.11** / **M2.3.12** / **M2.3.13** / **M2.3.14**: optional **`--p2p-listen ADDR`** binds a second TCP port, prints
...
//!   Integration tests
//!   (`tests/mfnd_smoke.rs`, M2.1.6.1 + M2.1.7 + M2.1.8 + M2.1.8.1 + M2.1.9 + M2.1.10 + M2.1.11 + M2.1.12 + M2.1.13 + M2.1.14 + M2.1.15 + M2.1.16 + M2.1.17 + M2.1.18 + M2.2.8 + M2.2.10 + M2.3.3 + M2.3.4 + M2.3.5 + M2.3.6 + M2.3.7 + M2.3.8 + M2.3.9 + M2.3.10 + M2.3.11 + M2.3.12 + M2.3.13 + M2.3.14) drive `serve` over TCP
