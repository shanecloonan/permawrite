...
[![Tests](https://img.shields.io/badge/tests-595_passing-brightgreen)](#status)
...
**Pre-network.** This is the consensus-critical primitive layer plus a working state-transition function. The `mfnd` binary exercises checkpoint persistence, optional JSON genesis (`--genesis`), solo **`step`** (mempool drain + coinbase + produce / apply / save, with **`--blocks N`** to advance multiple heights in one run), and env-gated validator seeds; there is no P2P network, JSON-RPC surface, or public testnet yet. As of the latest commit:
...
| **Total** | | **595** | Zero `unsafe`. Zero clippy warnings. Two ignored placeholders/doc examples. |
...
# Run the full workspace test suite (595 passing tests, ~6 min on a modern machine)
