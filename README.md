...
[![Tests](https://img.shields.io/badge/tests-412_passing-brightgreen)](#status)
...
| Light-client follower      | [`mfn-light`](./mfn-light/README.md) |   34  | **M2.0.6 header-chain follower + M2.0.7 body-root verification + M2.0.8 validator-set evolution** — `LightChain` tracks a tip pointer + a trusted validator set + the small shadow state needed to evolve that set across rotations (`validator_stats`, `pending_unbonds`, `BondEpochCounters`), all bootstrapped from `GenesisConfig`. `apply_header(&BlockHeader)` does strict-monotonicity + `prev_hash` linkage + cryptographic verification via M2.0.5's `verify_header`. `apply_block(&Block)` adds M2.0.7's body-root verification *plus* M2.0.8's validator-set evolution: calls the same four shared `validator_evolution` helpers `apply_block` uses, byte-for-byte. The chain's own headers audit the light client's evolution — if it ever drifts from the full node's view, the very next block's `verify_header` rejects with `ValidatorRootMismatch`. State byte-for-byte preserved on any failure; typed errors distinguish forged headers (`HeaderVerify`) from header-honest / body-tampered pairs (`BodyMismatch`) from Byzantine-bad bond ops (`EvolutionFailed`). Pure-Rust deps only (no `tokio`, no `rocksdb`) — same crate compiles to `wasm32-unknown-unknown` for browser wallets and bridges. Integration tests prove `LightChain` and `mfn-node::Chain` reach identical tips + identical `validator_set_root`s across a real **5-block rotation chain** (Register at block 1, Unbond at block 3, settlement at block 5). The light client now follows the chain indefinitely from a single genesis bootstrap. |
...
| **Total** | | **412** | Zero `unsafe`. Zero clippy warnings. |
