...
| [`network`](src/network.rs) | **M2.3.0** scaffold only: [`NetworkConfig`](src/network.rs) defaults for a future gossip listener + dialer. No sockets, no wire codec yet — reserves `mfn_node::network` for multi-node work without re-plumbing `lib.rs`. |
...
- **Unit (`network::tests`, M2.3.0)**: 1 test — [`NetworkConfig::default`](src/network.rs) (`listen_addr`, `max_outbound_peers`).
