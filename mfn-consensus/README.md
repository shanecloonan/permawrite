...
- **Integration** (`tests/integration.rs`, 20 tests) — multi-block flows: genesis → block1 → block2 with privacy tx, storage upload, slashing; full `unbond_lifecycle` with 3 validators, BLS finality, request → delay → settle, equivocation-during-delay still slashes, exit-churn cap spills across blocks; post-sign header tamper rejects for `validator_root`, `slashing_root`, `storage_proof_root`; legacy and validator mixed CLSAG fee + SPoRA forward-apply plus legacy/validator two-block treasury ledger identity and legacy/validator tampered-CLSAG rollback after prior valid history.
