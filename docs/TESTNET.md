...
- `.github/workflows/ci.yml::public-devnet scripts` plus `scripts/ci-check.{sh,ps1}` — parse-check Bash and PowerShell public-devnet helpers, then assert recovery walkthrough HTTP/P2P plan modes and opt-in proof safety text before operator-script regressions can land.
