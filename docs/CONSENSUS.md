...
- **`tests/integration.rs` (Agent 1 validator mixed integration, `30b35c0` / `2063202` / `56a53f3` / `302f3b7` / `e07d526`)** — `validator_mixed_clsag_fee_and_storage_proof_at_genesis_plus_block1` proves validator-mode BLS finality + coinbase + CLSAG fee + SPoRA forward-apply; `validator_mixed_two_block_treasury_ledger_identity` carries that invariant across two blocks and checks treasury credit/drain identity; `reject_validator_mixed_tampered_clsag_after_one_block_without_state_change` proves a tampered validator-mixed CLSAG after a prior valid block rejects without moving the tip; `reject_validator_mixed_tampered_storage_proof_root_after_one_block_without_state_change` proves post-seal `storage_proof_root` tamper after prior valid validator-mixed history rejects without moving the tip; `reject_validator_mixed_invalid_coinbase_after_one_block_without_state_change` proves underpaid validator coinbase after prior valid mixed history rejects without moving the tip.
